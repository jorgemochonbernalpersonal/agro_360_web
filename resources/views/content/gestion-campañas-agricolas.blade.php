        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="mb-12">
                <div class="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-teal-100 border border-teal-300 mb-6">
                    ...
                    <span class="text-sm font-semibold text-teal-800">{{ __('Organización Anual') }}</span>
                </div>
                <h1 class="text-5xl lg:text-6xl font-bold text-[var(--color-agro-green-dark)] mb-6">{{ __('Gestión de Campañas Agrícolas') }}</h1>
                <p class="text-xl text-gray-600 leading-relaxed">
                    <strong>{{ __('Organiza tu trabajo por campañas') }}</strong>: cada año agrícola con sus actividades, tratamientos, cosecha y facturación separados. Compara rendimientos y resultados entre campañas.
                </p>
            </div>

            <article class="prose prose-lg max-w-none">
                <section class="mb-12">
                    <h2 class="text-3xl font-bold text-[var(--color-agro-green-dark)] mb-4">{{ __('¿Qué es una Campaña Agrícola?') }}</h2>
                    <p class="text-gray-700 leading-relaxed mb-4">
                        Una <strong>{{ __('campaña agrícola') }}</strong> en viticultura típicamente va desde la poda (enero-febrero) hasta la vendimia (septiembre-octubre) del mismo año. En Agro365, cada campaña agrupa:
                    </p>
                ...
                <section class="mb-12">
                    <h2 class="text-3xl font-bold text-[var(--color-agro-green-dark)] mb-4">{{ __('Beneficios de la Gestión por Campañas') }}</h2>
                    <div class="grid md:grid-cols-2 gap-6 mb-6">
                        ...
                        <div class="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
                            <div class="text-3xl mb-3">📋</div>
                            <h3 class="font-bold text-lg text-[var(--color-agro-green-dark)] mb-2">{{ __('Informes por Campaña') }}</h3>
                        ...
                        <div class="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
                            <div class="text-3xl mb-3">📁</div>
                            <h3 class="font-bold text-lg text-[var(--color-agro-green-dark)] mb-2">{{ __('Organización') }}</h3>
                            <p class="text-gray-700">{{ __('Mantén separados los datos de cada año sin mezclas.') }}</p>
                        </div>
                        <div class="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
                            <div class="text-3xl mb-3">📈</div>
                            <h3 class="font-bold text-lg text-[var(--color-agro-green-dark)] mb-2">{{ __('Análisis Histórico') }}</h3>
                            <p class="text-gray-700">{{ __('Detecta tendencias a lo largo de los años.') }}</p>
                        </div>
                    </div>
                </section>
                ...
                <section class="mb-12">
                    ...
                    <ul class="list-disc list-inside space-y-3 text-gray-700 mb-6 ml-4">
                        <li><strong>{{ __('Crear campañas:') }}</strong> Define fecha inicio y fin de cada campaña</li>
                        <li><strong>{{ __('Campaña activa:') }}</strong> Trabaja siempre en la campaña actual</li>
                        <li><strong>{{ __('Cambiar entre campañas:') }}</strong> Consulta datos de campañas anteriores</li>
                        <li><strong>{{ __('Copiar datos:') }}</strong> Copia parcelas y plantaciones a nueva campaña</li>
                        <li><strong>{{ __('Cerrar campaña:') }}</strong> Bloquea edición de campañas cerradas</li>
                    </ul>
                </section>
                ...
                <section class="mb-12">
                    <div class="bg-gradient-to-r from-[var(--color-agro-green-bg)] to-[var(--color-agro-green-light)]/30 p-8 rounded-xl border border-[var(--color-agro-green)]/20">
                        <h3 class="text-2xl font-bold text-[var(--color-agro-green-dark)] mb-4">{{ __('📆 Organiza por Campañas') }}</h3>
                        <p class="text-gray-700 mb-6">
                            Separa cada año agrícola, compara resultados y genera informes de campaña. <strong>{{ __('3 meses gratis') }}</strong>.
                        </p>
                ...
                <section class="mb-12">
                    ...
                    <div class="space-y-6">
                        <div class="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
                            <h3 class="font-bold text-lg text-[var(--color-agro-green-dark)] mb-2">{{ __('¿Cuándo empieza una campaña agrícola?') }}</h3>
                            <p class="text-gray-700">{{ __('Típicamente con la poda en enero-febrero o cuando tú lo configures según tu explotación.') }}</p>
                        </div>
                        <div class="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
                            <h3 class="font-bold text-lg text-[var(--color-agro-green-dark)] mb-2">{{ __('¿Puedo tener varias campañas abiertas?') }}</h3>
                            <p class="text-gray-700">{{ __('Sí, aunque normalmente trabajas en una campaña activa, puedes consultar datos de campañas anteriores.') }}</p>
                        </div>
                        <div class="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
                            <h3 class="font-bold text-lg text-[var(--color-agro-green-dark)] mb-2">{{ __('¿Qué pasa cuando cierro una campaña?') }}</h3>
                            <p class="text-gray-700">{{ __('Se bloquea la edición de datos de esa campaña para preservar la integridad de los informes.') }}</p>
                        </div>
                        <div class="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
                            <h3 class="font-bold text-lg text-[var(--color-agro-green-dark)] mb-2">{{ __('¿Las parcelas se copian a la nueva campaña?') }}</h3>
                            <p class="text-gray-700">{{ __('Sí, al crear una nueva campaña puedes copiar automáticamente las parcelas y plantaciones.') }}</p>
                        </div>
                        <div class="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
                            <h3 class="font-bold text-lg text-[var(--color-agro-green-dark)] mb-2">{{ __('¿Puedo comparar rendimientos entre campañas?') }}</h3>
                            <p class="text-gray-700">{{ __('Sí, el sistema permite comparar datos de producción, costes y rendimientos año a año.') }}</p>
                        </div>
                    </div>
                </section>
            </article>
