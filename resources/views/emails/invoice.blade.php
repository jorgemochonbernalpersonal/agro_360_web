...
<head>
    ...
    <title>{{ $invoice->invoice_number ?? 'Albarán ' . $invoice->delivery_note_code }}</title>
</head>
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
    ...
    <div style="background: linear-gradient(135deg, #4d7c0f 0%, #3f6212 100%); padding: 30px; border-radius: 10px 10px 0 0; text-align: center;">
        <h1 style="color: white; margin: 0; font-size: 22px; font-weight: 700;">
            {{ $invoice->invoice_number ? 'Factura ' . $invoice->invoice_number : 'Albarán ' . $invoice->delivery_note_code }}
        </h1>
        @if($invoice->invoice_number && $invoice->delivery_note_code)
            <p style="color: #d9f99d; margin: 6px 0 0; font-size: 13px;">Albarán: {{ $invoice->delivery_note_code }}</p>
        @endif
    </div>
    ...
    <div style="background: #f9fafb; padding: 30px; border-radius: 0 0 10px 10px; border: 1px solid #e5e7eb; border-top: none;">
        ...
        <p style="font-size: 15px; margin-bottom: 20px;">
            Estimado/a <strong>{{ $invoice->client->full_name ?? ($invoice->billing_first_name . ' ' . $invoice->billing_last_name) }}</strong>,
        </p>

        <p style="font-size: 14px; color: #4b5563; margin-bottom: 24px;">
            Adjunto encontrará los documentos correspondientes
            @if($invoice->invoice_number)
                a la factura <strong>{{ $invoice->invoice_number }}</strong>
            @else
                al albarán <strong>{{ $invoice->delivery_note_code }}</strong>
            @endif
            con fecha {{ $invoice->invoice_date ? $invoice->invoice_date->format('d/m/Y') : now()->format('d/m/Y') }}.
        </p>

        {{-- Resumen --}}
        <div style="background: white; padding: 20px; border-radius: 8px; margin-bottom: 20px; border-left: 4px solid #4d7c0f;">
            <h3 style="margin-top: 0; color: #4d7c0f; font-size: 15px;">Resumen</h3>
            <table style="width: 100%; border-collapse: collapse; font-size: 13px;">
                @if($invoice->invoice_number)
                <tr>
                    <td style="padding: 5px 0; color: #6b7280; width: 45%;">Nº Factura</td>
                    <td style="padding: 5px 0; font-weight: bold;">{{ $invoice->invoice_number }}</td>
                </tr>
                @endif
                @if($invoice->delivery_note_code)
                <tr>
                    <td style="padding: 5px 0; color: #6b7280;">Albarán</td>
                ...
                <tr>
                    <td style="padding: 5px 0; color: #6b7280;">Base imponible</td>
        ...
        <div style="background: white; padding: 20px; border-radius: 8px; margin-bottom: 20px;">
            <h3 style="margin-top: 0; color: #374151; font-size: 14px;">Detalle</h3>
            <table style="width: 100%; border-collapse: collapse; font-size: 12px;">
                <thead>
                    <tr style="border-bottom: 1px solid #e5e7eb;">
                        <th style="text-align: left; padding: 6px 4px; color: #6b7280; font-weight: 600;">Concepto</th>
                        <th style="text-align: right; padding: 6px 4px; color: #6b7280; font-weight: 600;">Cant.</th>
                        <th style="text-align: right; padding: 6px 4px; color: #6b7280; font-weight: 600;">Precio</th>
                        <th style="text-align: right; padding: 6px 4px; color: #6b7280; font-weight: 600;">Total</th>
                    </tr>
                </thead>
                <tbody>
        ...
        <p style="font-size: 13px; color: #6b7280;">
            Los documentos PDF están adjuntos a este correo.
            @if($senderName)
                <br>Un saludo de <strong>{{ $senderName }}</strong>.
            @endif
        </p>

        <p style="font-size: 12px; color: #9ca3af; margin-top: 30px; text-align: center; border-top: 1px solid #e5e7eb; padding-top: 16px;">
