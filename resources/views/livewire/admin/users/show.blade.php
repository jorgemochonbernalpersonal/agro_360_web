        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-5">
            ...
            <div>
                ...
                <div class="flex items-center gap-2 mt-0.5">
                    ...
                    @else
                        <span class="text-sm text-zinc-400">{{ __('No fundador') }}</span>
                    @endif
                    <flux:button
                    ...
                    >{{ $user->is_founder ? __('Revocar') : __('Conceder') }}</flux:button>
                </div>
                @php $maxSlots = config('app.founder_max_slots', 50); $taken = \App\Models\User::where('is_founder', true)->count(); @endphp
                <p class="text-xs text-zinc-400 mt-1">{{ $taken }} / {{ $maxSlots }} plazas ocupadas</p>
            </div>
            @endif
            @if($user->isProducer())
            <div>
    ...
    <x-agro.card>
        ...
        @php
            $eventLabels = [
                ...
                'user_founder_toggled'            => ['label' => 'Fundador concedido/revocado',     'color' => 'amber',  'icon' => 'star'],
