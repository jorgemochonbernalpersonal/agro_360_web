<x-agro.form-card
    title="{{ __('Nueva Operación de Bodega') }}"
    :description="__('Registra un trasiego, clarificación, filtración u otra operación enológica.')"
    icon="calendar-days"
    ...
    <form wire:submit="save" class="space-y-8">
        ...
        <x-agro.form-section title="{{ __('Contenedores y Volumen') }}" color="blue">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <flux:field>
                    <flux:label required>{{ __('Volumen (litros)') }}</flux:label>
        ...
        <x-agro.form-section title="{{ __('Responsable y Notas') }}" color="green">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <flux:field>
                    ...
                    <flux:input wire:model="responsible_person" type="text" :placeholder="__('Nombre del responsable')" />
            ...
            <div class="mt-4">
                <flux:field>
                    ...
                    <flux:textarea wire:model="notes" rows="3" :placeholder="__('Observaciones, condiciones, resultados...')" />
                    <flux:error name="notes" />
                </flux:field>
            </div>
        </x-agro.form-section>

        <x-agro.form-actions :back-url="roleRoute('cellar-operations.index')" submit-:label="__('Guardar operación')" />
    </form>
</x-agro.form-card>
