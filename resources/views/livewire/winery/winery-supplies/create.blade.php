<x-agro.form-card
    title="{{ __('Nuevo Insumo de Bodega') }}"
    :description="__('Añade un producto al catálogo de insumos.')"
    icon="beaker"
    ...
    <form wire:submit="save" class="space-y-8">
        <x-agro.form-section title="{{ __('Identificación') }}" color="blue">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <flux:field class="md:col-span-2">
                    ...
                    <flux:input wire:model="name" type="text" :placeholder="__('Ej. Metabisulfito potásico')" required />
                    <flux:error name="name" />
                </flux:field>

                <flux:field>
                    <flux:label>{{ __('Nombre comercial') }}</flux:label>
                    <flux:input wire:model="commercial_name" type="text" :placeholder="__('Opcional')" />
        ...
        <x-agro.form-section title="{{ __('Notas') }}" color="green">
            <flux:field>
                ...
                <flux:textarea wire:model="notes" rows="2" :placeholder="__('Descripción, instrucciones de uso...')" />
                <flux:error name="notes" />
            </flux:field>
        </x-agro.form-section>

        <x-agro.form-actions :cancel-url="roleRoute('winery-supplies.index')" :submit-label="__('Guardar insumo')" />
    </form>
</x-agro.form-card>
