<div>
    <x-agro.form-card title="Editar Factura de Productos"
        description="{{ $invoice->invoice_number }} — {{ $invoice->client?->full_name }}"
        :back-url="route('winery.invoices.wine-sale.index')">

        @if ($isLocked)
            {{-- ── VISTA BLOQUEADA: solo estado de cobro editable ─────────────────── --}}
            @php
                $isDelivered = $invoice->delivery_status === 'delivered';
                $isCancelled = $invoice->status === 'cancelled';
            @endphp

            <flux:callout variant="{{ $isCancelled ? 'danger' : 'warning' }}" icon="lock-closed" class="mb-6">
                Esta factura está
                @if ($isCancelled) <strong>cancelada</strong> y no se puede modificar.
                @else <strong>entregada</strong>. Solo puedes actualizar el estado de cobro.
                @endif
            </flux:callout>

            {{-- Resumen de stock --}}
            @if ($invoice->items->count())
                <x-agro.form-section title="Resumen de productos">
                    <div class="space-y-3">
                        @foreach ($invoice->items as $item)
                            @php
                                $lot = $item->wineLot;
                                $stockLabel = $isDelivered ? 'Vendido' : ($isCancelled ? 'Devuelto a stock' : 'Reservado');
                                $stockColor = $isDelivered ? 'text-blue-600' : ($isCancelled ? 'text-zinc-500' : 'text-orange-600');
                            @endphp
                            <div class="flex items-center justify-between py-2 border-b border-zinc-100 last:border-0">
                                <div>
                                    <p class="font-medium text-zinc-900 text-sm">{{ $item->name }}</p>
                                    @if ($lot)
                                        <p class="text-xs text-zinc-400">{{ ucfirst($lot->wine_type) }}{{ $lot->vintage ? ' · ' . $lot->vintage : '' }} · {{ $lot->unit }}</p>
                                    @endif
                                </div>
                                <div class="text-right">
                                    <p class="font-bold text-zinc-900 text-sm">{{ number_format($item->quantity, 3) }} <span class="{{ $stockColor }} text-xs font-medium ml-1">{{ $stockLabel }}</span></p>
                                    <p class="text-xs text-zinc-400">{{ number_format($item->unit_price, 2) }} €/ud · IVA {{ number_format($item->tax_rate, 0) }}%</p>
                                </div>
                            </div>
                        @endforeach
                    </div>
                    <div class="flex justify-end mt-4">
                        <div class="w-56 space-y-1 text-sm">
                            <div class="flex justify-between text-zinc-500">
                                <span>Base imponible:</span><span>{{ number_format($invoice->tax_base, 2) }} €</span>
                            </div>
                            <div class="flex justify-between text-zinc-500">
                                <span>IVA:</span><span>{{ number_format($invoice->tax_amount, 2) }} €</span>
                            </div>
            ...
            @if (! $isCancelled)
                <form wire:submit.prevent="save" class="space-y-6 mt-6">
                    <x-agro.form-section title="Estado de cobro">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <flux:field>
                                <flux:label for="payment_status">Estado de cobro *</flux:label>
                                <flux:select wire:model="payment_status" id="payment_status">
                    ...
                    <x-agro.form-actions :cancel-url="route('winery.invoices.wine-sale.index')" submit-label="Guardar Estado" />
                </form>
            @endif
