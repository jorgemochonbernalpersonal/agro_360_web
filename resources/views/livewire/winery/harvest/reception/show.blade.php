<div class="space-y-6 animate-fade-in">
    ...
    <x-agro.page-header
        :title="__('Recepción #:id', ['id' => $harvest->id])"
    ...
    @if($isCancelled)
        <flux:callout variant="danger" icon="x-circle">
            <flux:callout.text>Esta recepción ha sido <strong>{{ __('anulada') }}</strong> y no se contabiliza en los totales.</flux:callout.text>
        </flux:callout>
    @elseif($harvest->disqualified)
        <flux:callout variant="warning" icon="exclamation-triangle">
            <flux:callout.text>
                <strong>{{ __('Uva descartada.') }}</strong>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <x-agro.card>
                    ...
                    <dl class="grid grid-cols-2 md:grid-cols-3 gap-x-6 gap-y-3 text-sm">
                        ...
                        @if($harvest->health_status)
                            <div>
                                <dt class="text-zinc-500">{{ __('Estado sanitario') }}</dt>
                    ...
                    @if($harvest->sanitary_state_grapes || $harvest->sanitary_state_agraces || $harvest->sanitary_state_botrytis || $harvest->sanitary_state_oidium || $harvest->sanitary_state_mildew)
                        <div class="mt-4 pt-4 border-t border-zinc-100">
                            <p class="text-xs font-medium text-zinc-500 mb-3">{{ __('Estado sanitario detallado (%)') }}</p>
                ...
                <x-agro.card>
                    <x-slot:header>
                        <div class="flex items-center gap-2">
                            ...
                            <span class="font-semibold text-zinc-900">{{ __('Observaciones') }}</span>
                        </div>
                    </x-slot:header>
        ...
        <div class="space-y-6">
            ...
            <x-agro.card>
                <x-slot:header>
                    <span class="font-semibold text-zinc-900">{{ __('Resumen') }}</span>
                </x-slot:header>
                <div class="space-y-3 text-sm">
                    <div class="flex justify-between">
                        <span class="text-zinc-500">{{ __('Estado') }}</span>
                        @if($isCancelled)
                            <flux:badge color="zinc">{{ __('Anulada') }}</flux:badge>
                        @elseif($harvest->disqualified)
                            <flux:badge color="red">{{ __('Descartada') }}</flux:badge>
                        @else
                            <flux:badge color="green">{{ __('Activa') }}</flux:badge>
                        @endif
                    </div>
                    <div class="flex justify-between">
                        <span class="text-zinc-500">{{ __('Depósito') }}</span>
                        @if($harvest->container)
                            <flux:badge color="green">{{ $harvest->container->name }}</flux:badge>
                        @else
                            <flux:badge color="zinc">{{ __('Sin asignar') }}</flux:badge>
                        @endif
                    </div>
                    <div class="flex justify-between">
                        <span class="text-zinc-500">{{ __('Registrada') }}</span>
                        <span class="text-zinc-700">{{ $harvest->created_at?->format('d/m/Y H:i') }}</span>
                    </div>
                    @if($harvest->wasEdited())
                        <div class="flex justify-between">
                            <span class="text-zinc-500">{{ __('Última edición') }}</span>
                            <span class="text-zinc-700">{{ $harvest->edited_at?->format('d/m/Y H:i') }}</span>
                        </div>
                        @if($harvest->editor)
                            <div class="flex justify-between">
                                <span class="text-zinc-500">{{ __('Editada por') }}</span>
            ...
            <x-agro.card>
                <x-slot:header>
                    <div class="flex items-center gap-2">
                        ...
                        <span class="font-semibold text-zinc-900">{{ __('Declaración del viticultor') }}</span>
                    </div>
                </x-slot:header>
                ...
                @if($delivery)
                    ...
                    <div class="space-y-2.5 text-sm">
                        ...
                        <div class="flex justify-between">
                            <span class="text-zinc-500">{{ __('Kg declarados') }}</span>
                        ...
                        @if($delivery->discrepancy_kg && $delivery->discrepancy_kg > 0)
                            <div class="flex justify-between">
                                <span class="text-zinc-500">{{ __('Diferencia') }}</span>
                                ...
                                @if($showResolveForm)
                                    <div class="bg-blue-50 border border-blue-200 rounded-xl px-3 py-3 space-y-2.5">
                                        ...
                                        <textarea
                                            ...
                                            rows="3"
                                            maxlength="1000"
                                            placeholder="{{ __('Explica el motivo de la diferencia: merma por transporte, corrección de pesaje, error de ticket...') }}"
                                        ...
                                        <div class="flex gap-2 justify-end">
                                            <flux:button wire:click="cancelResolveDispute" variant="ghost" size="sm">{{ __('Cancelar') }}</flux:button>
                                            <flux:button wire:click="resolveDispute" variant="primary" size="sm" icon="check-circle">{{ __('Enviar respuesta') }}</flux:button>
                                        </div>
                                    </div>
                                @else
                                    <button
                    ...
                    @endif
                @else
                    <p class="text-sm text-zinc-400">{{ __('El viticultor no ha declarado ninguna entrega enlazada con esta recepción.') }}</p>
                @endif
            </x-agro.card>
            ...
            @if(!$isCancelled)
                <x-agro.card>
                    <x-slot:header>
                        <span class="font-semibold text-zinc-900">{{ __('Acciones') }}</span>
                    </x-slot:header>
                    <div class="space-y-2">
