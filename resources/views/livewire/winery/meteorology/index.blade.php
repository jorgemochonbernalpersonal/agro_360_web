...
<x-agro.page-header
    ...
    :description="__('Datos meteorologicos en tiempo real de tus parcelas: temperatura, humedad, suelo, radiacion y pronostico.')"
    icon="cloud"
>
    <x-slot:actions>
        <div class="flex items-center gap-2">
            @if($selectedPlot)
                <flux:button wire:click="refreshData" size="sm" variant="ghost" icon="arrow-path">{{ __('Actualizar') }}</flux:button>
            @endif
        </div>
    </x-slot:actions>
</x-agro.page-header>
...
@if(!$selectedPlot)
    <x-agro.empty-state
        ...
        :description="__('Elige una parcela para ver sus datos meteorologicos en tiempo real.')"
    />
@elseif($error)
    <x-agro.empty-state
        icon="exclamation-triangle"
        :title="$error"
        :description="__('Intenta de nuevo mas tarde o selecciona otra parcela.')"
    />
@else
    {{-- Tiempo actual --}}
    <div>
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div class="bg-gradient-to-br from-indigo-50 to-purple-50 rounded-2xl p-5 ring-1 ring-indigo-200/60">
                <div class="flex items-center justify-between mb-2">
                    <span class="text-xs font-bold uppercase tracking-wider text-indigo-700">{{ __('Precipitacion') }}</span>
                ...
                <p class="text-xs text-zinc-500 mt-1">{{ __('Ultimas 24 horas') }}</p>
            </div>
    ...
    <div>
        <h3 class="text-sm font-semibold text-zinc-500 uppercase tracking-wider mb-3">{{ __('Suelo y radiacion') }}</h3>
            ...
            <x-agro.card>
                <div class="flex items-center justify-between mb-3">
                    <div class="flex items-center gap-2">
                        ...
                        <span class="text-sm font-semibold text-zinc-800">{{ __('Radiacion solar') }}</span>
                    </div>
                </div>
            ...
            <x-agro.card>
                <div class="flex items-center justify-between mb-3">
                    <div class="flex items-center gap-2">
                        ...
                        <span class="text-sm font-semibold text-zinc-800">{{ __('Estres hidrico') }}</span>
                    </div>
                </div>
                ...
                @if($waterStress['status'] === 'severe')
                    <div class="mt-3 p-2 rounded-lg bg-red-50 border border-red-200">
                        <p class="text-xs text-red-700 font-medium">{{ __('Se recomienda riego urgente para evitar danos en la vid.') }}</p>
                    </div>
                @elseif($waterStress['status'] === 'moderate')
                    <div class="mt-3 p-2 rounded-lg bg-orange-50 border border-orange-200">
                        <p class="text-xs text-orange-700 font-medium">{{ __('Considerar riego en las proximas 24-48 horas.') }}</p>
                    </div>
                @endif
            </x-agro.card>
        </div>
    ...
    <x-agro.card>
        ...
        @elseif(!$showForecast)
            <p class="text-sm text-zinc-400 text-center py-4">{{ __('Pulsa "Mostrar" para ver el pronostico de los proximos 7 dias.') }}</p>
        @else
            <p class="text-sm text-zinc-400 text-center py-4">{{ __('Sin datos de pronostico disponibles.') }}</p>
        @endif
    </x-agro.card>
    ...
    @if(isset($weather['mock']) && $weather['mock'])
        <div class="flex items-center justify-center gap-2 text-xs text-amber-600 bg-amber-50 rounded-xl p-2">
            ...
            <span>{{ __('Datos de demostracion — activa la API de Open-Meteo en produccion para datos reales.') }}</span>
        </div>
    @endif
@endif
