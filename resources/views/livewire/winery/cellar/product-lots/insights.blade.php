<div class="space-y-6 animate-fade-in">
    ...
    <x-agro.page-header title="{{ __('Insights de Lotes') }}" :description="__('Análisis de ventas por lote de producto y período')">
        <x-slot:actions>
            @if(!empty($pivot['lots']))
                <flux:button wire:click="export" icon="arrow-down-tray" variant="ghost">{{ __('Exportar Excel') }}</flux:button>
            @endif
            <flux:button :href="roleRoute('product-lots.index')" icon="arrow-left" variant="ghost">{{ __('Volver') }}</flux:button>
        </x-slot:actions>
    </x-agro.page-header>
    ...
    @if(empty($pivot['months']))
        <x-agro.empty-state
            icon="archive-box"
            title="{{ __('Sin datos') }}"
            :description="__('No hay ventas de lotes en el período seleccionado con los filtros aplicados.')"
        />
    @else
        @php
            $months     = $pivot['months'];
                ...
                <table class="min-w-full text-sm">
                    <thead>
                        <tr class="bg-zinc-50 border-b border-zinc-200">
                            <th class="text-left px-4 py-3 font-semibold text-zinc-700 min-w-[180px]">{{ __('Lote de producto') }}</th>
                            ...
                            @endforeach
                            <th class="text-right px-4 py-3 font-bold text-zinc-900 bg-violet-50 border-l border-violet-200">{{ __('Total') }}</th>
                        </tr>
                    </thead>
