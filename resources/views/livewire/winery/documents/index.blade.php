...
<x-agro.page-header
    title="{{ __('Documentos de Bodega') }}"
    :description="__('Gestión de licencias, permisos, certificados y otros documentos oficiales.')"
    icon="folder-open"
>
    <x-slot:actions>
...
<x-agro.filter-bar>
    <x-agro.filter-input wire:model.live="search" :placeholder="__('Buscar por título, referencia...')" />
                ...
                >
                                ...
                                <span class="text-zinc-700 font-medium">
                                    @if($document->expiry_date)
                                        ...
                                        @if($document->isExpired())
                                            <x-agro.status-badge :label="__('Caducado')" color="red" />
                                        @elseif($document->isExpiringSoon())
                                            <x-agro.status-badge :label="__('Próximo')" color="amber" />
                                        @endif
                                    @else
                                        —
                                    @endif
                    ...
                    <x-slot:footer>
                        <div class="flex items-center justify-end gap-0.5">
                            <x-agro.action-button icon="pencil" variant="default" href="{{ roleRoute('documents.edit', $document) }}" wire:navigate title="{{ __('Editar') }}" />
                            <x-agro.action-button variant="delete" wire:click="delete({{ $document->id }})" wire:loading.attr="disabled" wire:confirm="{{ __('¿Eliminar este documento?') }}" title="{{ __('Eliminar') }}" />
                        </div>
                    </x-slot:footer>
                </x-agro.card>
            @endforeach
