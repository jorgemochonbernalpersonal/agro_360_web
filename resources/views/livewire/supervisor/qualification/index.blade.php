<div class="space-y-6 animate-fade-in">
    ...
    <x-agro.page-header title="{{ __('Calificación') }}" :description="__('Registro de catas y calificación de vinos por añada.')">
        <x-slot name="actions">
            <flux:button wire:click="toggleCreate" variant="primary" icon="plus">{{ __('Registrar vino') }}</flux:button>
        </x-slot>
    </x-agro.page-header>

    {{-- Stats --}}
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <x-agro.stat-card :label="__('Total registrados')" :value="$tabs['all']['count']" icon="star" color="blue" />
        <x-agro.stat-card :label="__('Calificados')" :value="$tabs['qualified']['count']" icon="check-circle" color="agro" />
        <x-agro.stat-card :label="__('Pendientes')" :value="$tabs['pending']['count']" icon="clock" color="yellow" />
    </div>

    {{-- Create form --}}
    @if($showCreate)
    <x-agro.card>
        <h3 class="text-sm font-semibold text-zinc-700 mb-4">{{ __('Nuevo registro de calificación') }}</h3>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
                <flux:label>{{ __('Nombre del vino') }}</flux:label>
            ...
            <div>
                <flux:label>{{ __('Grado alcohólico (%)') }}</flux:label>
                <flux:input type="number" wire:model="alcohol_percentage" step="0.01" min="0" max="25" />
            </div>
            <div>
                <flux:label>{{ __('Fecha de cata') }}</flux:label>
                <flux:input type="date" wire:model="qualification_date" />
            </div>
            <div class="sm:col-span-2">
                <flux:label>{{ __('Notas de cata') }}</flux:label>
                <flux:textarea wire:model="tasting_notes" rows="2" />
            </div>
        </div>
        <div class="flex gap-3 mt-4">
            <flux:button wire:click="saveQualification" variant="primary">{{ __('Guardar') }}</flux:button>
                        ...
                        >
                            ...
                            <div class="flex-1 space-y-4">
                                <div class="grid grid-cols-2 gap-2">
                                    ...
                                    <div class="bg-agro-50 rounded-xl p-3">
                                        <p class="text-[10px] font-semibold text-agro-400 uppercase tracking-widest mb-0.5">{{ __('Alcohol') }}</p>
                                ...
                                <div class="space-y-2 text-sm">
                                    <div class="flex items-center justify-between">
                                        <span class="text-zinc-400">{{ __('Color') }}</span>
                                        <span class="text-zinc-700 font-medium capitalize">{{ $colorLabels[$q->color] ?? '—' }}</span>
                                    </div>
                                    <div class="flex items-center justify-between">
                                        <span class="text-zinc-400">{{ __('Fecha cata') }}</span>
                            ...
                            <x-slot:footer>
                                <div class="flex items-center justify-end gap-1 flex-wrap">
                                    ...
                                    @endif
                                    <x-agro.action-button icon="pencil" variant="default" wire:click="openEdit({{ $q->id }})" title="{{ __('Editar') }}" />
                                </div>
                            </x-slot:footer>
                        </x-agro.card>
                    @endforeach
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40" wire:key="edit-modal-{{ $editId }}">
        <div class="bg-white rounded-2xl shadow-xl w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto">
            <div class="flex items-center justify-between px-6 py-4 border-b border-zinc-100">
                <h3 class="text-base font-semibold text-zinc-800">{{ __('Editar calificación') }}</h3>
            ...
            <div class="p-6 space-y-4">
                ...
                <p class="text-xs font-semibold text-zinc-400 uppercase tracking-wide pt-2">{{ __('Parámetros analíticos') }}</p>
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
                    <div>
                        <flux:label>{{ __('Alcohol (%)') }}</flux:label>
                ...
                <p class="text-xs font-semibold text-zinc-400 uppercase tracking-wide pt-2">{{ __('Puntuaciones de cata (0–10)') }}</p>
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
                    <div>
                        <flux:label>{{ __('Visual') }}</flux:label>
            ...
            <div class="flex justify-end gap-3 px-6 py-4 border-t border-zinc-100">
                ...
                <flux:button wire:click="updateQualification" variant="primary">{{ __('Guardar cambios') }}</flux:button>
            </div>
        </div>
    </div>
    @endif
