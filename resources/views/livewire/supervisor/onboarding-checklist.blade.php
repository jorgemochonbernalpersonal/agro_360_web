    ...
    @if($show)
        <div x-data="{ expanded: false }" class="mb-4">
            ...
            <div @click="expanded = !expanded"
                 class="bg-gradient-to-r from-indigo-50 to-purple-50 rounded-lg border border-indigo-200 p-3 cursor-pointer hover:shadow-md transition-all">
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-full bg-indigo-600 flex items-center justify-center text-white text-lg">
                            🚀
                        </div>
                        <div>
                            ...
                            <div class="flex items-center gap-2 mt-0.5">
                                ...
                                <span class="text-xs font-medium text-indigo-700">{{ $progressPercentage }}%</span>
                            </div>
                        </div>
                    </div>
                    ...
                    @foreach($steps as $step)
                        <div class="flex items-start gap-3 p-2 rounded hover:bg-zinc-50 transition-colors">
                            ...
                            @if(!$step['completed'])
                                <a href="{{ $step['route'] }}"
                                   wire:navigate
                                   class="flex-shrink-0 px-2 py-1 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-medium rounded transition-colors">
                                    Ir
                                </a>
                            @endif
                        </div>
