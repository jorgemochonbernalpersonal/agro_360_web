<div class="space-y-6 animate-fade-in">
    ...
    {{-- ACTIVE / INACTIVE TABS --}}
        ...
        @php
            $filterCount = (int)(!empty($filterAutonomousCommunity))
            ...
            <div class="flex flex-wrap items-center gap-2">
                @if($filterAutonomousCommunity)
                    <span class="inline-flex items-center gap-1.5 pl-3 pr-2 py-1 bg-agro-50 text-agro-700 text-xs font-medium rounded-full border border-agro-200">
                        ...
                        <button wire:click="$set('filterAutonomousCommunity', ''); $set('filterProvince', ''); $set('filterMunicipality', '')" class="ml-0.5 p-0.5 rounded-full hover:bg-agro-200 transition-colors">
                ...
                @if($filterProvince)
                    <span class="inline-flex items-center gap-1.5 pl-3 pr-2 py-1 bg-agro-50 text-agro-700 text-xs font-medium rounded-full border border-agro-200">
                        ...
                        <button wire:click="$set('filterProvince', ''); $set('filterMunicipality', '')" class="ml-0.5 p-0.5 rounded-full hover:bg-agro-200 transition-colors">
                            <flux:icon icon="x-mark" class="size-3" />
                        </button>
                    </span>
                @endif
                @if($filterMunicipality)
                    <span class="inline-flex items-center gap-1.5 pl-3 pr-2 py-1 bg-agro-50 text-agro-700 text-xs font-medium rounded-full border border-agro-200">
                        ...
                        <button wire:click="$set('filterMunicipality', '')" class="ml-0.5 p-0.5 rounded-full hover:bg-agro-200 transition-colors">
                            <flux:icon icon="x-mark" class="size-3" />
                        </button>
                    </span>
                @endif
        ...
        <div wire:loading wire:target="switchTab, search, filterAutonomousCommunity, filterProvince, filterMunicipality, nextPage, previousPage, gotoPage">
        ...
        <div wire:loading.remove wire:target="switchTab, search, filterAutonomousCommunity, filterProvince, filterMunicipality, nextPage, previousPage, gotoPage">
            @if($plots->count() > 0)
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-2">
    ...
    <x-agro.modal name="plot-audit" maxWidth="3xl">
        <div class="bg-zinc-50 px-6 py-4 border-b border-zinc-200 rounded-t-xl">
            <div class="flex items-center justify-between">
                ...
                <flux:button
                    x-on:click="$dispatch('close-modal', 'plot-audit')"
