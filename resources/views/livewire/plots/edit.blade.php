<div>
<x-agro.form-card title="Editar Parcela" description="Modifica los datos de la parcela" :back-url="roleRoute('plots.index')">
                ...
                <flux:field>
                    ...
                    <flux:select wire:change="selectCommunity($event.target.value)" id="autonomous_community_id"
                        ...
                        @foreach ($autonomousCommunities as $community)
                            <option value="{{ $community->id }}" @selected($autonomous_community_id == $community->id)>{{ $community->name }}</option>
                        @endforeach
                    </flux:select>
