    ...
    @if(!$tokenValid)
        ...
        <x-agro.card>
            <div class="text-center py-6 space-y-4">
                ...
                <flux:heading size="lg">{{ __('Enlace no válido') }}</flux:heading>
                <flux:subheading>
                    {{ __('Este enlace de activación ha expirado, ya fue usado, o no es correcto. Contacta con quien te envió la invitación para solicitar una nueva.') }}
                </flux:subheading>
                <flux:button href="{{ route('login') }}" variant="ghost" wire:navigate>
                    {{ __('Ir al inicio de sesión') }}
                </flux:button>
            </div>
        </x-agro.card>
    ...
    @else
        ...
        <x-agro.card>
            ...
            <div class="text-center mb-6">
                ...
                <flux:heading size="xl">{{ __('Activa tu cuenta') }}</flux:heading>
                <flux:subheading>
                    @if($invitorType === 'supervisor')
                        {{ __('La denominación de origen ya ha configurado tus parcelas y plantaciones.') }}<br>
                    @else
                        {{ __('Tu bodega ya ha configurado tus parcelas y plantaciones.') }}<br>
                    @endif
                    {{ __('Elige una contraseña para acceder a tu cuaderno de campo.') }}
                </flux:subheading>
            </div>
            ...
            <div class="space-y-4">
                        ...
                        <label class="flex items-start gap-3 cursor-pointer">
                            ...
                            <div>
                                <p class="text-sm font-medium text-agro-900">
                                    {{ __('Compartir mi cuaderno de campo con :winery', ['winery' => $wineryName]) }}
                                </p>
                                <p class="text-xs text-agro-700 mt-0.5">
                                    @if($invitorType === 'supervisor')
                                        {{ __('La denominación de origen podrá ver tus tratamientos, riegos y labores culturales.') }}
                                    @else
                                        {{ __('La bodega podrá ver tus tratamientos, riegos y labores culturales.') }}
                                    @endif
                                    {{ __('Puedes revocar este acceso en cualquier momento desde Configuración.') }}
                                </p>
                            </div>
                        </label>
                    </div>
                ...
                <flux:button
                    ...
                    class="w-full"
                >
                    <span wire:loading.remove>{{ __('Activar cuenta y entrar') }}</span>
                    <span wire:loading>{{ __('Activando...') }}</span>
                </flux:button>
