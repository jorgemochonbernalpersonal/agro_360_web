<div>
    <x-agro.page-header
        ...
        :description="__('Visión completa viñedo + bodega: del esfuerzo en campo al resultado en bodega')"
    ...
    <div class="flex flex-wrap items-end gap-4 mb-6">
        <flux:field class="min-w-[250px]">
            ...
            <flux:select wire:model.live="filterCampaign">
                ...
                @foreach($campaigns as $c)
                    <option value="{{ $c->id }}">{{ $c->name }} ({{ $c->year }}) {{ $c->active ? '● ' . __('Activa') : '' }}</option>
                @endforeach
            </flux:select>
        </flux:field>
        ...
        @if($campaign)
            <div class="flex items-center gap-2 pb-1">
                <flux:badge size="sm" :color="$campaign->active ? 'green' : 'zinc'">{{ $campaign->active ? __('Activa') : __('Cerrada') }}</flux:badge>
            </div>
        @endif
    </div>
    ...
    @if(!$filterCampaign)
        <x-agro.empty-state
            ...
            :description="__('Elige una campaña para ver el dashboard integrado viñedo + bodega.')"
        />
    @elseif(!$data)
        <x-agro.empty-state icon="chart-bar-square" :title="__('Sin datos')" :description="__('No hay datos para esta campaña.')" />
    @else
        ...
        <div class="bg-gradient-to-r from-green-50 via-amber-50 to-purple-50 rounded-2xl p-5 ring-1 ring-zinc-200/60 mb-6">
            <p class="text-xs font-bold uppercase tracking-wider text-zinc-500 mb-3">{{ __('Viñedo → Bodega') }}</p>
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-6">
                <div>
                    <p class="text-xs text-zinc-500">{{ __('Campo') }}</p>
                    <p class="text-xl font-bold text-green-700">{{ number_format($data['fieldHarvest']->total_kg ?? 0) }} kg</p>
                </div>
                <div>
                    <p class="text-xs text-zinc-500">{{ __('Recepción bodega') }}</p>
                    <p class="text-xl font-bold text-amber-700">{{ number_format($data['receptionStats']->total_kg ?? 0) }} kg</p>
                </div>
                <div>
                    <p class="text-xs text-zinc-500">{{ __('Merma campo→bodega') }}</p>
                    <p class="text-xl font-bold {{ $data['yieldLoss'] > 5 ? 'text-red-600' : 'text-zinc-700' }}">{{ $data['yieldLoss'] }}%</p>
                </div>
                <div>
                    <p class="text-xs text-zinc-500">{{ __('Multiplicador valor') }}</p>
                    <p class="text-xl font-bold text-purple-700">{{ $data['valueMultiplier'] }}x</p>
                    <p class="text-[10px] text-zinc-400">{{ __('ventas / valor campo') }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="mb-8">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-4">
                <x-agro.stat-card :label="__('Actividades')" :value="$data['totalActivities']" icon="pencil-square" color="green" />
                <x-agro.stat-card :label="__('Vendimias')" :value="$data['fieldHarvest']->entries ?? 0" icon="archive-box-arrow-down" color="green" />
                <x-agro.stat-card :label="__('Rend. medio')" :value="number_format($data['fieldHarvest']->avg_yield ?? 0) . ' kg/ha'" icon="chart-bar" color="blue" />
                <x-agro.stat-card :label="__('Costes parcela')" :value="number_format($data['plotCosts'], 0) . ' €'" icon="banknotes" color="orange" />
            </div>
                ...
                <x-agro.card>
                    <x-slot:header>
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-sm font-semibold text-zinc-800">{{ __('Cuaderno de campo') }}</span>
                        </div>
                    </x-slot:header>
                    @php
                        $actLabels = [
                            'phytosanitary'           => __('Tratamientos'),
                            'phytosanitary_treatment' => __('Tratamientos'),
                            'fertilization'           => __('Fertilizaciones'),
                            'irrigation'              => __('Riegos'),
                            'cultural'                => __('Labores'),
                            'cultural_work'           => __('Labores'),
                            'observation'             => __('Observaciones'),
                            'harvest'                 => __('Vendimia'),
                            'pruning'                 => __('Podas'),
                            'phenology'               => __('Fenología'),
                            'post_harvest'            => __('Post-vendimia'),
                            'post_harvest_treatment'  => __('Post-vendimia'),
                        ];
                    @endphp
                    <div class="space-y-1.5">
                        ...
                        @empty
                            <p class="text-sm text-zinc-400">{{ __('Sin actividades registradas.') }}</p>
                        @endforelse
                    </div>
                </x-agro.card>
                ...
                <x-agro.card>
                    <x-slot:header>
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-sm font-semibold text-zinc-800">{{ __('Calidad media de vendimia') }}</span>
                        </div>
                    </x-slot:header>
                ...
                <x-agro.card class="mt-4">
                    <x-slot:header>
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-sm font-semibold text-zinc-800">{{ __('Rendimiento por parcela') }}</span>
                        </div>
                    </x-slot:header>
                        ...
                        <table class="w-full text-sm">
                            <thead>
                                <tr class="bg-zinc-50 text-zinc-500 text-xs uppercase">
                                    ...
                                    <th class="text-right px-3 py-2">{{ __('Total kg') }}</th>
                                    <th class="text-right px-3 py-2">{{ __('Rend. medio') }}</th>
                                    <th class="text-right px-3 py-2">{{ __('Baumé') }}</th>
                                </tr>
                            </thead>
        ...
        <div>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-4">
                <x-agro.stat-card :label="__('Vinos')" :value="count($data['wines'])" icon="beaker" color="purple" />
                <x-agro.stat-card :label="__('Volumen en depósito')" :value="number_format($data['wineVolume']) . ' L'" icon="arrows-right-left" color="blue" />
                <x-agro.stat-card :label="__('Uso contenedores')" :value="$data['containerPct'] . '%'" icon="cube" color="orange" />
                <x-agro.stat-card :label="__('Lotes producto')" :value="$data['productStats']->lots ?? 0" icon="archive-box" color="green" />
            </div>
                ...
                <x-agro.card>
                    <x-slot:header>
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-sm font-semibold text-zinc-800">{{ __('Vinos por tipo') }}</span>
                        </div>
                    </x-slot:header>
                    @php
                    ...
                    <div class="space-y-2">
                        ...
                        @empty
                            <p class="text-sm text-zinc-400">{{ __('Sin vinos registrados.') }}</p>
                        @endforelse
                    </div>
                </x-agro.card>
                ...
                <x-agro.card>
                    <x-slot:header>
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-sm font-semibold text-zinc-800">{{ __('Estado de vinos') }}</span>
                        </div>
                    </x-slot:header>
                    @php
                        $statusLabels = ['in_progress' => __('En elaboración'), 'aging' => __('Crianza'), 'ready' => __('Listo'), 'bottled' => __('Embotellado'), 'archived' => __('Archivado')];
                    ...
                    <div class="space-y-2">
                        ...
                        @empty
                            <p class="text-sm text-zinc-400">{{ __('Sin datos de estado.') }}</p>
                        @endforelse
                    </div>
                </x-agro.card>
            </div>
            ...
            <x-agro.card class="mt-4">
                <x-slot:header>
                    <div class="flex items-center gap-2">
                        ...
                        <span class="text-sm font-semibold text-zinc-800">{{ __('Capacidad de bodega') }}</span>
                    </div>
                </x-slot:header>
                <div class="flex items-center gap-4">
                    ...
                    <div class="text-right shrink-0">
                        ...
                        <p class="text-xs text-zinc-500">{{ $data['containerPct'] }}% {{ __('ocupado') }}</p>
                    </div>
                </div>
            </x-agro.card>
