<div>
    ...
    @if($paginatedPlots->isEmpty())
        <x-agro.empty-state icon="map" :title="__('Sin parcelas')" :description="__('No hay parcelas que coincidan con los filtros seleccionados.')" />
    @else
        @php
            ...
            @foreach($paginatedPlots as $plot)
                <x-agro.card wire:key="plot-{{ $plot->id }}">
                    ...
                    @if($plot->plantings->isEmpty())
                        <p class="text-sm text-zinc-400">{{ __('Sin plantaciones activas.') }}</p>
                    @else
                        <div class="overflow-x-auto">
                            <table class="w-full text-sm">
                                <thead>
                                    <tr class="text-xs text-zinc-500 uppercase bg-zinc-50/50">
                                        <th class="text-left px-3 py-2">{{ __('Plantación') }}</th>
                                        <th class="text-left px-3 py-2">{{ __('Variedad') }}</th>
                                        <th class="text-center px-3 py-2">{{ __('Edad') }}</th>
                                        <th class="text-center px-3 py-2">{{ __('Etapa') }}</th>
                                        <th class="text-center px-3 py-2">{{ __('Fenología') }}</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-zinc-100">
                                    @foreach($plot->plantings as $planting)
                                        ...
                                        <tr class="hover:bg-zinc-50/50">
                                            ...
                                            <td class="px-3 py-2 text-center text-zinc-600">{{ $planting->age ?? '—' }} {{ __('años') }}</td>
                                            ...
                                            <td class="px-3 py-2 text-center">
                                                ...
                                                @else
                                                    <span class="text-xs text-zinc-400">{{ __('Sin observar') }}</span>
                                                @endif
                                            </td>
                                        </tr>
                                    @endforeach
