<div>
    <x-agro.form-card title="{{ __('Editar Cliente') }}" :description="__('Modifica los datos del cliente')"
        ...
        <form wire:submit.prevent="update" class="space-y-8">
            ...
            <x-agro.form-section title="{{ __('Datos de Contacto') }}">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    @if ($client_type === 'individual')
                        <flux:field>
                            <flux:label for="first_name">{{ __('Nombre') }} <span class="text-red-500">*</span></flux:label>
                            <flux:input wire:model="first_name" type="text" id="first_name" required />
                            <flux:error name="first_name" />
                        </flux:field>
                        <flux:field>
                            <flux:label for="last_name">{{ __('Apellidos') }}</flux:label>
                            <flux:input wire:model="last_name" type="text" id="last_name" />
                            <flux:error name="last_name" />
                        </flux:field>
                        <flux:field>
                            <flux:label for="particular_document">{{ __('DNI / NIE') }}</flux:label>
                            <flux:input wire:model="particular_document" type="text" id="particular_document" />
                            <flux:error name="particular_document" />
                        </flux:field>
                    @else
                        <flux:field>
                            <flux:label for="company_name">{{ __('Razón Social') }} <span class="text-red-500">*</span></flux:label>
                            <flux:input wire:model="company_name" type="text" id="company_name" required />
                            <flux:error name="company_name" />
                        </flux:field>
                        <flux:field>
                            <flux:label for="company_document">{{ __('CIF / NIF Empresa') }}</flux:label>
                    ...
                    <flux:field>
                        <flux:label for="phone">{{ __('Teléfono') }}</flux:label>
            ...
            <x-agro.form-section title="{{ __('Datos de Pago') }}">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <flux:field>
                        <flux:label for="default_discount">{{ __('Descuento por defecto (%)') }}</flux:label>
                    ...
                    <flux:field>
                        <flux:label for="payment_method">{{ __('Método de pago') }}</flux:label>
                    ...
                    <flux:field>
                        <flux:label for="account_number">{{ __('Número de cuenta (IBAN)') }}</flux:label>
            ...
            <x-agro.form-actions :cancel-url="roleRoute('clients.index')" submit-:label="__('Guardar Cambios')" />
        </form>
    </x-agro.form-card>
</div>
