            ...
            <div class="space-y-6">
                ...
                @foreach ($sigpacCodes as $index => $sigpac)
                    @php
                        ...
                        // Formato correcto: CA(2)-Prov(2)-Mun(3)-Agr(3)-Zona(3)-Pol(3)-Parc(5)-Rec(3)
                    ...
                    <div
                        x-data="{
                            ...
                            parsePaste(index) {
                                ...
                                let fields = {};
                                if (parts.length === 7) {
                                    // Estándar SIGPAC: Prov-Mun-Agr-Zona-Pol-Parc-Rec (sin CA)
                                    fields = {
                                    ...
                                    // Con CA: CA-Prov-Mun-Agr-Zona-Pol-Parc-Rec
                                    fields = {
                        ...
                        <div class="px-6 py-4 border-b border-zinc-100 flex items-center justify-between
                            ...
                            <div class="flex items-center gap-3">
                                <div class="flex items-center justify-center w-10 h-10 rounded-full font-bold text-sm
                                    ...
                                    @else bg-zinc-300 text-zinc-600 @endif">
                                    {{ $index + 1 }}
                                </div>
                                <div>
                                    <h3 class="text-lg font-semibold text-zinc-800">Código SIGPAC #{{ $index + 1 }}</h3>
                                    ...
                                    @elseif($hasDuplicate)
                                        <p class="text-xs text-red-700 font-medium flex items-center gap-1 mt-0.5">
                                            <flux:icon icon="x-circle" class="size-4" /> Duplicado: Polígono, Parcela y Recinto ya existen en otro código
                                        </p>
                                    @elseif($hasErrors)
                                        <p class="text-xs text-red-700 font-medium flex items-center gap-1 mt-0.5">
                            ...
                            @if(count($sigpacCodes) > 1)
                                <flux:button type="button" wire:click="removeSigpacCode({{ $index }})" variant="ghost" size="sm" icon="trash" class="text-red-600 hover:bg-red-50" />
                            @endif
                        </div>
                                ...
                                @foreach($fieldDefs as $field)
                                    ...
                                    <div>
                                        <label class="block text-xs font-semibold text-zinc-600 mb-1">
                                            {{ $field['label'] }}
                                            <span class="font-normal text-zinc-400">({{ $field['hint'] }})</span>
                                        </label>
                                        <input
                                            type="text"
