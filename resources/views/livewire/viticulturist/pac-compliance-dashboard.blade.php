<div class="space-y-6 animate-fade-in">
    ...
    <x-agro.page-header
        :title="__('Dashboard de Cumplimiento PAC')"
        :description="__('Monitoriza el estado de cumplimiento normativo de tu cuaderno digital en tiempo real')"
    />

    {{-- Selector de período --}}
    <div class="flex items-center gap-3">
        <span class="text-sm font-medium text-zinc-600">{{ __('Período de análisis:') }}</span>
        <flux:select wire:model.live="timeRange" class="w-48">
            ...
            <flux:select.option value="180">{{ __('Últimos 6 meses') }}</flux:select.option>
            <flux:select.option value="365">{{ __('Último año') }}</flux:select.option>
            <flux:select.option value="all">{{ __('Todas las actividades') }}</flux:select.option>
        </flux:select>
    </div>
    ...
    <x-agro.card>
        <x-slot:header>
            <div class="flex items-center gap-3">
                ...
                <span class="font-semibold text-zinc-900">{{ __('Cumplimiento Global') }}</span>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div class="text-center p-4 bg-blue-50 rounded-xl border border-blue-100">
                ...
                <div class="text-xs text-blue-600 mt-1 font-medium">{{ __('Total Actividades') }}</div>
            </div>
            <div class="text-center p-4 bg-red-50 rounded-xl border border-red-100">
                ...
                <div class="text-xs text-red-600 mt-1 font-medium">{{ __('Con Errores Críticos') }}</div>
            </div>
            <div class="text-center p-4 bg-amber-50 rounded-xl border border-amber-100">
                ...
                <div class="text-xs text-amber-600 mt-1 font-medium">{{ __('Con Advertencias') }}</div>
            </div>
        </div>
    </x-agro.card>
    ...
    <x-agro.card>
        <x-slot:header>
            <div class="flex items-center gap-2">
                ...
                <span class="font-semibold text-zinc-900">{{ __('Productos Fitosanitarios') }}</span>
            </div>
        </x-slot:header>
        ...
        <div class="grid grid-cols-2 gap-4 mb-5">
            <div class="text-center p-4 bg-blue-50 rounded-xl border border-blue-100">
                ...
                <div class="text-xs text-blue-600 mt-1 font-medium">{{ __('Total Productos') }}</div>
            </div>
            <div class="text-center p-4 bg-green-50 rounded-xl border border-green-100">
                <div class="text-2xl font-bold text-green-900">{{ $productsWithValidRegistration }}</div>
                <div class="text-xs text-green-600 mt-1 font-medium">{{ __('Registro Válido') }}</div>
            ...
            <div class="text-center p-4 bg-amber-50 rounded-xl border border-amber-100">
                ...
                <div class="text-xs text-amber-600 mt-1 font-medium">{{ __('Próximos a Caducar') }}</div>
                <div class="text-xs text-amber-500 mt-0.5">{{ __('Próximos 30 días') }}</div>
            </div>
            <div class="text-center p-4 bg-red-50 rounded-xl border border-red-100">
                <div class="text-2xl font-bold text-red-900">{{ $productsExpiredOrRevoked }}</div>
                <div class="text-xs text-red-600 mt-1 font-medium">{{ __('Caducados/Revocados') }}</div>
            </div>
        </div>

        <div class="mb-4">
            <div class="flex items-center justify-between mb-2">
                <span class="text-sm font-medium text-zinc-700">{{ __('Estado de Registros') }}</span>
        ...
        @if($productsExpiringSoon > 0)
            <flux:callout variant="warning" icon="exclamation-triangle">
                <flux:heading>{{ __('Productos próximos a caducar') }}</flux:heading>
    ...
    <x-agro.card>
        <x-slot:header>
            <div class="flex items-center gap-2">
                ...
                <span class="font-semibold text-zinc-900">{{ __('Actividades Bloqueadas') }}</span>
            </div>
        </x-slot:header>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-5">
            <div class="text-center p-4 bg-zinc-50 rounded-xl border border-zinc-200">
                ...
                <div class="text-xs text-zinc-600 mt-1 font-medium">{{ __('Total Bloqueadas') }}</div>
            </div>
            <div class="text-center p-4 bg-blue-50 rounded-xl border border-blue-100">
                <div class="text-2xl font-bold text-blue-900">{{ $lockedActivitiesPercentage }}%</div>
                <div class="text-xs text-blue-600 mt-1 font-medium">{{ __('% Bloqueadas') }}</div>
            </div>
            <div class="text-center p-4 bg-purple-50 rounded-xl border border-purple-100">
                ...
                <div class="text-xs text-purple-600 mt-1 font-medium">{{ __('Bloqueadas Recientemente') }}</div>
                <div class="text-xs text-purple-500 mt-0.5">{{ __('Últimos 7 días') }}</div>
            </div>
        </div>
        ...
        @if($recentlyLockedActivities->count() > 0)
            <div class="border border-zinc-200 rounded-xl overflow-hidden mb-4">
                <div class="bg-zinc-50 px-4 py-2 border-b border-zinc-200">
                    <p class="text-xs font-semibold text-zinc-600 uppercase tracking-wide">{{ __('Actividades bloqueadas recientemente') }}</p>
                </div>
                            ...
                            <div class="flex items-center justify-between">
                                <div>
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="text-xs text-zinc-600">{{ $activity->plot->name ?? __('Sin parcela') }}</span>
                                    </div>
                                    <p class="text-xs text-zinc-400 mt-0.5">
                                        {{ __('Fecha:') }} {{ $activity->activity_date->format('d/m/Y') }} · {{ __('Bloqueada:') }} {{ $activity->locked_at->diffForHumans() }}
                                    </p>
                                </div>
        ...
        @else
            <p class="text-sm text-zinc-500 text-center py-4">{{ __('No hay actividades bloqueadas recientemente') }}</p>
        @endif

        <flux:callout variant="info" icon="information-circle">
            {{ __('Las actividades se bloquean automáticamente después de 7 días para garantizar el cumplimiento PAC y prevenir modificaciones retroactivas.') }}
        </flux:callout>
    </x-agro.card>
        ...
        <x-agro.card>
            <x-slot:header>
                <div class="flex items-center gap-2">
                    ...
                    <span class="font-semibold text-zinc-900">{{ __('Validación de Plazos de Seguridad en Cosechas') }}</span>
                </div>
            </x-slot:header>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-5">
                <div class="text-center p-4 bg-blue-50 rounded-xl border border-blue-100">
                    ...
                    <div class="text-xs text-blue-600 mt-1 font-medium">{{ __('Total Cosechas') }}</div>
                    <div class="text-xs text-blue-500 mt-0.5">{{ __('En el período seleccionado') }}</div>
                </div>
                <div class="text-center p-4 bg-red-50 rounded-xl border border-red-100">
                    <div class="text-2xl font-bold text-red-900">{{ $harvestsWithWithdrawalIssues }}</div>
                    <div class="text-xs text-red-600 mt-1 font-medium">{{ __('Con Errores de Plazo') }}</div>
                    @if($harvestsWithWithdrawalIssues > 0)
                        <div class="text-xs text-red-500 mt-0.5">{{ __('Requieren atención') }}</div>
                    @endif
                </div>
                <div class="text-center p-4 bg-amber-50 rounded-xl border border-amber-100">
                    <div class="text-2xl font-bold text-amber-900">{{ $harvestsWithWarnings }}</div>
                    <div class="text-xs text-amber-600 mt-1 font-medium">{{ __('Con Advertencias') }}</div>
                    <div class="text-xs text-amber-500 mt-0.5">{{ __('Cerca del límite') }}</div>
                </div>
            </div>

            @if($harvestsWithWithdrawalIssues > 0)
                <flux:callout variant="danger" icon="x-circle">
                    <flux:heading>{{ __('Se detectaron') }} {{ $harvestsWithWithdrawalIssues }} {{ __('cosecha(s) que no cumplen el plazo de seguridad') }}</flux:heading>
                    {{ __('Estas cosechas se realizaron antes de cumplirse el plazo de seguridad del último tratamiento fitosanitario. Revisa el cuaderno digital para más detalles.') }}
                </flux:callout>
            @else
                <flux:callout variant="success" icon="check-circle">
                    {{ __('Todas las cosechas cumplen con los plazos de seguridad.') }}
                </flux:callout>
            @endif
        </x-agro.card>
    @endif
    ...
    <x-agro.card>
        <x-slot:header>
            <div class="flex items-center gap-2">
                ...
                <span class="font-semibold text-zinc-900">{{ __('Cumplimiento por Tipo de Actividad') }}</span>
            </div>
        </x-slot:header>

        @if(empty($statsByType))
            <p class="text-zinc-500 text-center py-8">{{ __('No hay actividades registradas en este período') }}</p>
        @else
                ...
                @foreach($statsByType as $type => $stats)
                    @php
                        $typeLabels = [
                            ...
                            'irrigation'    => __('Riegos'),
                            'fertilization' => __('Fertilizaciones'),
                            'harvest'       => __('Cosechas'),
                            'cultural'      => __('Labores Culturales'),
                            'observation'   => __('Observaciones'),
                        ];
                    @endphp
        ...
        <x-agro.card>
            <x-slot:header>
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-2">
                        ...
                        <span class="font-semibold text-red-800">{{ __('Errores Críticos PAC') }}</span>
                    </div>
                    <flux:badge color="red" size="sm">
                        {{ count($criticalErrors) }} {{ count($criticalErrors) === 1 ? __('error') : __('errores') }}
                    </flux:badge>
                </div>
            </x-slot:header>
                    ...
                    <div class="bg-red-50 border-l-4 border-red-500 p-4 rounded-r-xl">
                        <div class="flex items-start justify-between gap-3 mb-2">
                            <p class="text-sm font-medium text-red-800">
                                {{ __('Actividad #') }}{{ $error['activity_id'] }} — {{ ucfirst($error['activity_type']) }}
                            </p>
            ...
            @if($activitiesWithErrors > 10)
                <p class="text-sm text-zinc-500 mt-4 text-center">
                    {{ __('Mostrando 10 de') }} {{ $activitiesWithErrors }} {{ __('errores.') }}
                    <a href="{{ roleRoute('viticulturist.digital-notebook') }}" class="text-agro-600 hover:underline font-medium">{{ __('Ver todas las actividades') }}</a>
                </p>
            @endif
        </x-agro.card>
    @endif
    ...
    @if(count($warnings) > 0)
        <x-agro.card>
            <x-slot:header>
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-2">
                        ...
                        <span class="font-semibold text-amber-800">{{ __('Advertencias PAC') }}</span>
                    </div>
                    <flux:badge color="yellow" size="sm">
                        {{ count($warnings) }} {{ count($warnings) === 1 ? __('advertencia') : __('advertencias') }}
                    </flux:badge>
                </div>
            </x-slot:header>
                    ...
                    <div class="bg-amber-50 border-l-4 border-amber-500 p-4 rounded-r-xl">
                        <div class="flex items-start justify-between gap-3 mb-2">
                            <p class="text-sm font-medium text-amber-800">
                                {{ __('Actividad #') }}{{ $warning['activity_id'] }} — {{ ucfirst($warning['activity_type']) }}
                            </p>
            ...
            @if($activitiesWithWarnings > 10)
                <p class="text-sm text-zinc-500 mt-4 text-center">
                    {{ __('Mostrando 10 de') }} {{ $activitiesWithWarnings }} {{ __('advertencias.') }}
                </p>
            @endif
        </x-agro.card>
    @endif
    ...
    @if($compliancePercentage >= 95 && $activitiesWithErrors === 0)
        <flux:callout variant="success" icon="check-circle">
            <flux:heading>{{ __('¡Excelente Cumplimiento PAC!') }}</flux:heading>
            {{ __('Tu cuaderno digital cumple con todos los requisitos normativos. Todas tus actividades están correctamente registradas y listas para auditorías PAC.') }}
        </flux:callout>
    @endif
