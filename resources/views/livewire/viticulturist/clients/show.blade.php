<div class="space-y-6 animate-fade-in">
    <x-agro.page-header
        :title="__('Cliente: :name', ['name' => $client->full_name])"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            @if($client->payment_method)
                <div>
                    ...
                    <p class="font-semibold">{{ match($client->payment_method) { 'cash' => __('Efectivo'), 'transfer' => __('Transferencia'), 'check' => __('Cheque'), default => __('Otro') } }}</p>
                </div>
            @endif
        ...
        <x-slot:header>
            <div class="flex items-center justify-between w-full">
                ...
                @if($client->addresses && $client->addresses->count() > 0)
                    <flux:badge color="blue" size="sm">{{ $client->addresses->count() }} {{ $client->addresses->count() === 1 ? __('dirección') : __('direcciones') }}</flux:badge>
                @endif
            </div>
        </x-slot:header>
                                ...
                                <div class="flex items-center gap-2 mb-2">
                                    <h4 class="font-bold text-zinc-900">
                                        @if($address->description)
                                            {{ $address->description }}
                                        @else
                                            {{ __('Dirección #:num', ['num' => $loop->iteration]) }}
                                        @endif
                                    </h4>
