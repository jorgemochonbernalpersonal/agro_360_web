...
<div class="bg-white rounded-xl border border-zinc-200 p-4">
    <div class="flex items-center justify-between">
        <span class="text-sm text-zinc-500">{{ __('Valor total vendimia') }}</span>
        <span class="text-lg font-bold text-zinc-900">{{ number_format($h->total_value, 2, ',', '.') }} €</span>
    </div>
    <div class="flex items-center justify-between mt-1">
        <span class="text-sm text-zinc-500">{{ __('Entradas de vendimia') }}</span>
...
<div class="bg-white rounded-xl border border-zinc-200 p-4">
    <h4 class="text-xs font-semibold text-zinc-400 uppercase tracking-wide mb-3">{{ __('Actividades del Cuaderno') }} ({{ $data['totalActivities'] }})</h4>
    <div class="space-y-2">
