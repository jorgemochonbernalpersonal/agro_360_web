<div>
<x-agro.form-card
    :title="__('Nueva Observación Fenológica')"
    :description="__('Registra un estadio fenológico para una plantación. Si ya existe un registro para ese estadio/plantación/campaña, se actualizará.')"
...
>
    <form wire:submit.prevent="save" class="space-y-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <flux:field>
                    ...
                    @if($plantingLocked)
                        ...
                        <div class="flex items-center gap-2 px-3 py-2 bg-zinc-50 border border-zinc-200 rounded-lg text-sm text-zinc-700">
                            ...
                            <span>{{ $locked?->plot->name }} — {{ $locked?->grapeVariety->name ?? $locked?->name ?? __('Sin nombre') }}</span>
                        </div>
                        <input type="hidden" wire:model="plot_planting_id" />
                    @else
                        <flux:select wire:model.live="plot_planting_id">
                            <option value="">{{ __('Selecciona plantación') }}</option>
                            @foreach($plantings as $planting)
                                <option value="{{ $planting->id }}">
                                    {{ $planting->plot->name }} — {{ $planting->grapeVariety->name ?? $planting->name ?? __('Sin nombre') }}
                                </option>
                            @endforeach
                        </flux:select>
                    @endif
                ...
                <flux:field>
                    ...
                    <flux:select wire:model="campaign_id">
                        <option value="">{{ __('Selecciona campaña') }}</option>
        ...
        <x-agro.form-actions :cancel-url="$plantingLocked ? roleRoute('viticulturist.phenology.index', ['filter_planting_id' => $plot_planting_id]) : route('plots.plantings.index')" :submit-label="__('Guardar Observación')" />
    </form>
</x-agro.form-card>
</div>
