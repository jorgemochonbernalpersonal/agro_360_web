<div class="space-y-6 animate-fade-in">
    ...
    <x-agro.filter-section title="Filtros de Búsqueda" color="green">
        ...
        <div class="flex items-center">
            <label class="flex items-center space-x-2 cursor-pointer">
                ...
                >
                <span class="text-sm font-medium text-zinc-700">Solo en riesgo ahora</span>
            </label>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @foreach($pests as $pest)
                <a href="{{ route('viticulturist.pest-management.show', $pest) }}" 
                   class="bg-white rounded-lg shadow-sm border border-zinc-200 hover:shadow-md hover:border-agro-400 transition-all p-6 group">
                    
                    {{-- Header --}}
                    <div class="flex items-start justify-between mb-3">
                        <div class="flex-1">
                            <h3 class="font-semibold text-zinc-900 text-lg group-hover:text-agro-700 transition-colors">{{ $pest->name }}</h3>
                            @if($pest->scientific_name)
                                <p class="text-xs text-zinc-500 italic mt-1">{{ $pest->scientific_name }}</p>
                            @endif
                        </div>
                    ...
                    <div class="flex items-center justify-between pt-3 border-t border-zinc-100">
                        ...
                        <span class="text-xs text-agro-600 font-medium group-hover:translate-x-1 transition-transform inline-flex items-center gap-1">
                            Ver detalles
    ...
    @else
        <div class="bg-white rounded-lg shadow-sm border border-zinc-200 p-12 text-center">
            ...
            <h3 class="mt-4 text-lg font-medium text-zinc-900">No se encontraron resultados</h3>
            <p class="mt-2 text-zinc-600">Intenta ajustar los filtros de búsqueda</p>
        </div>
    @endif
</div>
