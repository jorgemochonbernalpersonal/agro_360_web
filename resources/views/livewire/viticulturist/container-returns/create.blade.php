<x-agro.form-card
    :title="__('Registrar Entrega de Envases')"
    :description="__('Documenta la entrega de envases vacíos de fitosanitarios en punto de recogida autorizado (SIGFITO / FIELD)')"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <flux:field>
                    ...
                    <flux:select wire:model.live="phytosanitary_product_id">
                        <flux:select.option value="">{{ __('Seleccionar producto...') }}</flux:select.option>
                        @foreach($products as $p)
                            <flux:select.option value="{{ $p->id }}">{{ $p->name }}</flux:select.option>
                        @endforeach
                    </flux:select>
                    <flux:description>{{ __('Seleccionar rellena automáticamente nombre y nº registro') }}</flux:description>
                </flux:field>
        ...
        <x-agro.form-section :title="__('Punto de Recogida')">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <flux:field>
                    ...
                    <flux:input wire:model="collection_point" type="text" :placeholder="__('Ej: Almacén Agrícola García - Valladolid')" />
