...
<x-agro.form-card
    ...
    :description="__('Registra un nuevo contenedor para tu bodega')"
    ...
    <form wire:submit="save" class="space-y-8">
        <x-agro.form-section :title="__('Información Básica')">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <flux:field>
                    ...
                    <flux:input wire:model="name" type="text" id="name" :placeholder="__('Ej: Barrica #1')" required />
            ...
            <div class="mt-6">
                <flux:field>
                    ...
                    <flux:textarea wire:model="description" id="description" rows="3" :placeholder="__('Descripción opcional del contenedor')" />
        ...
        <x-agro.form-section :title="__('Ubicación y Gestión')">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                @else
                    <div>
                        <p class="text-sm font-semibold text-zinc-700 mb-2">{{ __('Sala/Bodega') }}</p>
                        <flux:button
                            type="button"
                            variant="ghost"
                            icon="plus"
                        ...
                        >
                            {{ __('Crear Primera Sala/Bodega') }}
                        </flux:button>
                        <p class="text-xs text-zinc-500 mt-1">{{ __('Crea una sala para organizar tus contenedores') }}</p>
                    </div>
                @endif
...
<div id="createRoomModal" class="hidden fixed inset-0 bg-zinc-900 bg-opacity-50 z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-xl shadow-xl max-w-md w-full p-6">
        <div class="flex items-center justify-between mb-4">
            <h3 class="text-lg font-semibold text-zinc-900">{{ __('Nueva Sala/Bodega') }}</h3>
            <button type="button"
        ...
        <form wire:submit.prevent="createRoom" class="space-y-4">
            <flux:field>
                <flux:label>{{ __('Nombre de la Sala') }} *</flux:label>
                <flux:input wire:model="room_name" type="text" id="room_name" :placeholder="__('Ej: Bodega Principal')" required />
            </flux:field>
            <flux:field>
                <flux:label>{{ __('Descripción') }}</flux:label>
                <flux:textarea wire:model="room_description" id="room_description" rows="2" :placeholder="__('Descripción opcional')" />
            </flux:field>
