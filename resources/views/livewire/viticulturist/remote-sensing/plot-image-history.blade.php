<div class="bg-white rounded-xl shadow-sm border border-zinc-200 p-6">
    <div class="flex items-center justify-between mb-6">
        <h3 class="text-lg font-semibold text-zinc-900 flex items-center gap-2">
            🛰️ {{ __('Historial de Imágenes Satélite') }}
        </h3>
        <flux:select wire:model.live="months" >
            <option value="3">{{ __('Últimos 3 meses') }}</option>
            <option value="6">{{ __('Últimos 6 meses') }}</option>
            <option value="12">{{ __('Último año') }}</option>
            <option value="24">{{ __('Últimos 2 años') }}</option>
        </flux:select>
    </div>
    ...
    @if(count($history) > 0)
        ...
        @if($selectedIndex !== null && isset($history[$selectedIndex]))
            ...
            <div class="bg-gradient-to-br from-zinc-50 to-white rounded-xl p-6 border border-zinc-100 mb-6">
                <div class="flex items-center justify-between mb-4">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <div class="font-semibold text-zinc-900">{{ $selected['date'] }}</div>
                            <div class="text-sm text-zinc-500">{{ $selected['image_source'] }}</div>
                        </div>
                    </div>
                    <div class="text-right">
                        <div class="text-sm text-zinc-500">{{ __('Estado') }}</div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="text-center p-3 bg-white rounded-lg shadow-sm">
                        ...
                        <div class="text-xs text-zinc-400">{{ $selected['trend_icon'] }}</div>
                    </div>
                    <div class="text-center p-3 bg-white rounded-lg shadow-sm">
                        <div class="text-xs text-zinc-500 mb-1">NDWI</div>
                        <div class="text-xl font-bold text-blue-600">
                            {{ $selected['ndwi_formatted'] }}
                        </div>
                    </div>
                    <div class="text-center p-3 bg-white rounded-lg shadow-sm">
                        <div class="text-xs text-zinc-500 mb-1">{{ __('Temperatura') }}</div>
                        <div class="text-xl font-bold text-zinc-900">
                            {{ $selected['temperature'] !== null ? number_format($selected['temperature'], 1) . '°C' : 'N/A' }}
                        </div>
                    </div>
                    <div class="text-center p-3 bg-white rounded-lg shadow-sm">
                        <div class="text-xs text-zinc-500 mb-1">{{ __('Nubosidad') }}</div>
                        <div class="text-xl font-bold text-zinc-900">
    ...
    @else
        <div class="text-center py-12 text-zinc-500">
            ...
            <h4 class="text-lg font-medium mb-1">{{ __('Sin historial de imágenes') }}</h4>
            <p class="text-sm">{{ __('No hay datos de satélite para el período seleccionado') }}</p>
        </div>
    @endif
</div>
