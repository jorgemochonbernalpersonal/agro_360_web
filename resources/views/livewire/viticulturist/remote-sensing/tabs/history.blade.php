...
<div wire:key="history-tab-content-{{ $selectedSigpacId ?? 'none' }}-{{ $historyPeriod }}">
    ...
    <div class="mb-6 bg-white rounded-lg border border-zinc-200 p-4">
        <div class="flex flex-col md:flex-row gap-4">
            <div class="flex-1">
                <label class="block text-sm font-medium text-zinc-700 mb-2">📅 {{ __('Período de análisis') }}</label>
                <flux:select wire:model.live="historyPeriod">
                    <option value="7_days">{{ __('Últimos 7 días') }}</option>
                    <option value="30_days">{{ __('Últimos 30 días') }}</option>
                    <option value="90_days">{{ __('Últimos 90 días (por defecto)') }}</option>
                    <option value="current_season">{{ __('Temporada actual (Abril - Hoy)') }}</option>
                    <option value="last_season">{{ __('Temporada anterior (Abril - Octubre)') }}</option>
                    <option value="1_year">{{ __('Último año') }}</option>
            ...
            @if($historyPeriod === 'custom')
                <div class="flex-1">
                    <label class="block text-sm font-medium text-zinc-700 mb-2">{{ __('Rango personalizado') }}</label>
                    ...
                    <p class="text-xs text-zinc-500 mt-1">{{ __('Máximo: 2 años de rango') }}</p>
                </div>
            @endif
        </div>
        ...
        <div class="mt-3 flex items-center gap-4 text-sm text-zinc-600">
            <span class="flex items-center gap-1">
                ...
                {{ __('Mostrando:') }} <strong>{{ $historyDays }} {{ __('días') }}</strong>
            </span>
            @if(count($historicalData) > 0)
                <span class="flex items-center gap-1">
                    ...
                    <strong>{{ count($historicalData) }}</strong> {{ __('registros') }}
                </span>
            @endif
        </div>
    </div>
    ...
    {{-- Gráfico NDVI --}}
    <h3 class="text-md font-semibold mb-3">📈 {{ __('Evolución NDVI') }}</h3>
    ...
    @if(count($historicalData) > 0)
        <div class="bg-white rounded-lg border border-zinc-200 p-4 mb-4">
                ...
                @foreach($historicalData as $data)
                    ...
                    <div class="flex-1 flex flex-col items-center group relative {{ $data['high_clouds'] ? 'opacity-40' : '' }}">
                        <div class="absolute bottom-full mb-2 hidden group-hover:block bg-zinc-900 text-white text-xs rounded py-2 px-3 z-10 whitespace-nowrap">
                            ...
                            @if($data['high_clouds'])
                                <div class="text-orange-400 mt-1">☁️ {{ $data['cloud_coverage'] }}% {{ __('nubes') }} — {{ __('dato poco fiable') }}</div>
                            @else
                                <div class="text-zinc-400">☁️ {{ $data['cloud_coverage'] }}% {{ __('nubes') }}</div>
                            @endif
                        </div>
            ...
            @if($highCloudDays > 0)
                <p class="mt-2 text-xs text-orange-600 flex items-center gap-1">
                    ...
                    {{ $highCloudDays }} {{ $highCloudDays === 1 ? __('fecha') : __('fechas') }} {{ __('con >60% nubes aparecen atenuadas — datos poco fiables') }}
                </p>
            @endif
        </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
            ...
            <div class="bg-purple-50 border border-purple-200 rounded-lg p-3 text-center">
                <div class="text-xs text-purple-600 mb-1">{{ __('Variabilidad') }}</div>
        ...
        <div class="flex flex-wrap gap-3 mb-6">
            ...
            <button wire:click="toggleComparison"
                ...
                {{ $showComparison ? __('Ocultar') : __('Comparar') }} {{ __('Períodos') }}
            </button>
        </div>
        ...
        @if($showComparison)
            <div class="bg-purple-50 border border-purple-200 rounded-lg p-4 mb-6">
                <div class="flex items-center justify-between mb-4">
                    <h4 class="text-lg font-semibold text-purple-900">📊 {{ __('Comparación de Períodos') }}</h4>
                    <flux:select wire:model.live="comparisonPeriod">
                        <option value="last_year">{{ __('Mismo período año anterior') }}</option>
                        <option value="last_season">{{ __('Temporada anterior') }}</option>
                        <option value="same_month_last_year">{{ __('Mismo mes año anterior') }}</option>
                    </flux:select>
                </div>
                ...
                @if(count($comparisonData) > 0)
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="bg-white rounded-lg p-4">
                            <h5 class="text-sm font-semibold text-zinc-700 mb-3">{{ __('Período Actual') }}</h5>
                            <div class="space-y-2 text-sm">
                                <div class="flex justify-between">
                                    <span class="text-zinc-600">{{ __('NDVI Promedio:') }}</span>
                                    <span class="font-bold text-green-600">{{ number_format($avgNdvi, 3) }}</span>
                                </div>
                                <div class="flex justify-between">
                                    <span class="text-zinc-600">{{ __('Máximo:') }}</span>
                                    <span class="font-semibold">{{ number_format($maxNdvi, 3) }}</span>
                                </div>
                                <div class="flex justify-between">
                                    <span class="text-zinc-600">{{ __('Registros:') }}</span>
                        ...
                        <div class="bg-white rounded-lg p-4">
                            <h5 class="text-sm font-semibold text-zinc-700 mb-3">{{ __('Período Comparación') }}</h5>
                            @php
                    ...
                    <div class="mt-4 bg-white rounded-lg p-4 text-center">
                        <div class="text-sm text-zinc-600 mb-1">{{ __('Diferencia Promedio') }}</div>
                        ...
                        <div class="text-sm mt-2">
                            @if($avgDiff > 0.05)     <span class="text-green-600">✅ {{ __('Mejora significativa respecto al período anterior') }}</span>
                            @elseif($avgDiff < -0.05)<span class="text-red-600">⚠️ {{ __('Reducción respecto al período anterior') }}</span>
                            @else                     <span class="text-zinc-600">➡️ {{ __('Valores similares al período anterior') }}</span>
                            @endif
                        </div>
                    </div>
                @else
                    <p class="text-center text-purple-700 py-4">{{ __('No hay datos disponibles para el período de comparación') }}</p>
                @endif
            </div>
        @endif
        ...
        @if(count($periodAlerts) > 0)
            <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
                <h4 class="text-lg font-semibold text-yellow-900 mb-3">⚠️ {{ __('Alertas Detectadas') }} ({{ count($periodAlerts) }})</h4>

                <div class="mb-3">
                    <label class="text-sm text-zinc-700 font-medium">{{ __('Umbral NDVI:') }}</label>
                    <input type="range"
                    ...
                    @foreach($periodAlerts as $alert)
                        <div class="flex items-start gap-3 p-3 bg-white rounded-lg {{ $alert['severity'] === 'critical' ? 'border-l-4 border-red-500' : 'border-l-4 border-yellow-500' }}">
                            ...
                            <div class="flex-1">
                                ...
                                <div class="text-xs text-zinc-600 mt-1">{{ __('Tipo:') }} {{ $alert['type'] }}</div>
                            </div>
                        </div>
                    @endforeach
                </div>
        ...
        @if(!empty($trendPrediction))
            <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <h4 class="text-lg font-semibold text-blue-900 mb-3">🔮 {{ __('Predicción de Tendencia (próximos 7 días)') }}</h4>

                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                    <div class="bg-white rounded-lg p-3 text-center">
                        <div class="text-xs text-zinc-600 mb-1">{{ __('Tendencia') }}</div>
                        <div class="text-2xl font-bold {{ $trendPrediction['trend'] === 'improving' ? 'text-green-600' : ($trendPrediction['trend'] === 'declining' ? 'text-red-600' : 'text-zinc-600') }}">
                            @if($trendPrediction['trend'] === 'improving')    ↗️ {{ __('Mejorando') }}
                            @elseif($trendPrediction['trend'] === 'declining') ↘️ {{ __('Declinando') }}
                            @else                                              → {{ __('Estable') }}
                            @endif
                        </div>
                    </div>
                    <div class="bg-white rounded-lg p-3 text-center">
                        <div class="text-xs text-zinc-600 mb-1">{{ __('Pendiente') }}</div>
                        <div class="text-2xl font-bold text-blue-900">{{ number_format($trendPrediction['slope'], 4) }}</div>
                    </div>
                    <div class="bg-white rounded-lg p-3 text-center">
                        <div class="text-xs text-zinc-600 mb-1">{{ __('Confianza (R²)') }}</div>
                        <div class="text-2xl font-bold text-purple-900">{{ number_format($trendPrediction['confidence'] * 100, 1) }}%</div>
                    </div>
                </div>

                <div class="bg-white rounded-lg p-4">
                    <h5 class="text-sm font-semibold text-zinc-700 mb-3">{{ __('Valores Predichos:') }}</h5>
                ...
                <div class="mt-3 text-xs text-blue-700 bg-white rounded p-2">
                    <strong>{{ __('Nota:') }}</strong> {{ __('Las predicciones se basan en regresión lineal de los datos históricos. Confianza alta (>80%) indica patrón consistente. Úsalo solo como guía orientativa.') }}
                </div>
            </div>
        @endif
    ...
    @else
        <div class="text-center py-12 bg-zinc-50 rounded-lg border border-zinc-200">
            ...
            <p class="text-zinc-600 font-medium mb-1">{{ __('No hay datos históricos') }}</p>
            <p class="text-sm text-zinc-500">{{ __('Para el período seleccionado') }}</p>
        </div>
    @endif
