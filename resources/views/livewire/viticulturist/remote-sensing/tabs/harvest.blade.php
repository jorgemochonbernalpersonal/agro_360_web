...
<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
    ...
    <div class="bg-gradient-to-br from-pink-50 to-purple-50 rounded-lg p-5 border border-pink-200">
        ...
        <div class="text-center py-4">
            ...
            <div class="flex justify-between text-xs text-zinc-500 mt-1">
                <span>{{ __('Brotación') }}</span>
                <span>{{ __('Vendimia') }}</span>
            </div>
        </div>
    </div>
    ...
    <div class="bg-gradient-to-br from-amber-50 to-orange-50 rounded-lg p-5 border border-amber-200">
        <h3 class="font-semibold text-zinc-800 mb-4 flex items-center gap-2">
            🌡️ {{ __('Grados-Día Acumulados (GDD)') }}
        </h3>
        <div class="space-y-4">
            <div class="bg-white rounded-lg p-3 shadow-sm">
                <div class="flex justify-between items-center">
                    <span class="text-sm text-zinc-600">{{ __('GDD Hoy') }}</span>
                    <span class="font-bold text-orange-600">+{{ $gdd['gdd_today'] }}°</span>
                </div>
            </div>
            <div class="bg-white rounded-lg p-3 shadow-sm">
                <div class="flex justify-between items-center">
                    <span class="text-sm text-zinc-600">{{ __('GDD Próximos 7 días') }}</span>
                    <span class="font-bold text-amber-600">+{{ $gdd['gdd_week_forecast'] }}°</span>
                </div>
            </div>
            <div class="bg-white rounded-lg p-3 shadow-sm">
                <div class="flex justify-between items-center">
                    <span class="text-sm text-zinc-600">{{ __('GDD Acumulado (desde 1 abril)') }}</span>
                ...
                <div class="text-xs text-zinc-500 mt-1 text-right">{{ $gdd['gdd_accumulated'] }} / {{ $gdd['gdd_target'] }}° {{ __('objetivo') }}</div>
            </div>
            @if($gdd['estimated_harvest_date'])
                <div class="bg-purple-100 rounded-lg p-4 text-center">
                    <div class="text-sm text-purple-600">{{ __('Fecha estimada de vendimia') }}</div>
                    <div class="text-xl font-bold text-purple-800">📅 {{ $gdd['estimated_harvest_date'] }}</div>
                    <div class="text-xs text-purple-600">({{ __('en ~') }}{{ $gdd['days_to_harvest'] }} {{ __('días') }})</div>
                </div>
            @endif
        </div>
    </div>
