<div class="bg-white rounded-lg shadow-lg p-6">
    ...
    @elseif($isLoading)
        <div class="flex items-center justify-center py-8">
            <svg class="w-8 h-8 animate-spin text-agro-600" fill="none" viewBox="0 0 24 24">
                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path>
            </svg>
            <span class="ml-2 text-zinc-600">{{ __('Cargando datos de satélite...') }}</span>
        </div>
    @elseif($latestData)
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
            ...
            <div class="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-lg p-4 border border-blue-200">
                <div class="flex items-center justify-between mb-2">
                    <span class="text-sm font-semibold text-zinc-600">{{ __('Tendencia') }}</span>
                ...
                <span class="inline-block px-2 py-1 text-xs font-semibold rounded-full mt-2
                    ...
                    @elseif($latestData->trend === 'stable') bg-zinc-100 text-zinc-800
                    @else bg-red-100 text-red-800
                    @endif">
                    @if($latestData->trend === 'increasing') {{ __('En aumento') }}
                    @elseif($latestData->trend === 'stable') {{ __('Estable') }}
                    @else {{ __('En descenso') }}
                    @endif
                </span>
            </div>
            ...
            <div class="bg-gradient-to-br from-purple-50 to-violet-50 rounded-lg p-4 border border-purple-200">
                <div class="flex items-center justify-between mb-2">
                    <span class="text-sm font-semibold text-zinc-600">{{ __('Última imagen') }}</span>
                ...
                <div class="mt-2 flex items-center gap-2 text-xs text-zinc-600">
                    <span>☁️ {{ $latestData->cloud_coverage ?? 0 }}% {{ __('nubes') }}</span>
                </div>
        ...
        <div class="bg-gradient-to-r from-amber-50 to-orange-50 rounded-lg p-4 border border-amber-200 mb-4">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h4 class="font-semibold text-zinc-800">{{ __('Comparativa vs Año Anterior') }}</h4>
                        <p class="text-xs text-zinc-500">{{ __('Mismo mes de') }} {{ now()->year - 1 }}</p>
                    </div>
                </div>
                <div class="text-right">
                    <div class="flex items-center gap-4">
                        <div class="text-center">
                            <div class="text-sm text-zinc-500">{{ now()->year - 1 }}</div>
                            <div class="text-xl font-bold text-zinc-600">{{ number_format($lastYearNdvi, 2) }}</div>
                        </div>
                        <div class="text-2xl">→</div>
                        <div class="text-center">
                            <div class="text-sm text-zinc-500">{{ now()->year }}</div>
        ...
        <div class="bg-zinc-50 rounded-lg p-4 mb-4">
            <p class="text-sm text-zinc-700">
                <strong>{{ __('Análisis:') }}</strong> {{ $latestData->health_notes }}
            </p>
            @if($latestData->metadata && isset($latestData->metadata['mock_data']) && $latestData->metadata['mock_data'])
                <p class="text-xs text-amber-600 mt-2 flex items-center gap-1">
                    ...
                    {{ __('Datos de demostración. Conecta con Copernicus para datos reales.') }}
                </p>
            @endif
        </div>
        ...
        <div class="flex justify-between items-center">
            <button wire:click="toggleChart"
                ...
                {{ $showChart ? __('Ocultar histórico') : __('Ver histórico (90 días)') }}
            </button>

            <a href="{{ route('remote-sensing.dashboard') }}"
               class="text-sm text-agro-600 hover:text-agro-800 flex items-center gap-1 transition">
                {{ __('Ver todas las parcelas') }}
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        @if($showChart && count($historicalData) > 0)
            <div class="mt-4 p-4 bg-white border rounded-lg">
                <h3 class="text-sm font-semibold text-zinc-700 mb-3">📈 {{ __('Evolución NDVI - Últimos 90 días') }}</h3>
    ...
    @else
        <div class="text-center py-8 bg-zinc-50 rounded-lg">
            ...
            <p class="text-zinc-600 mb-2">{{ __('No hay datos de teledetección disponibles') }}</p>
            <p class="text-sm text-zinc-500">{{ __('Los datos se generarán automáticamente cuando haya imágenes satelitales disponibles para esta parcela.') }}</p>
        </div>
    @endif
</div>
