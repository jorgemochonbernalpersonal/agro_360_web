<div class="bg-white rounded-xl shadow-sm border border-zinc-200 p-6">
    <div class="flex items-center justify-between mb-4">
        ...
        <button wire:click="reloadData" class="text-zinc-400 hover:text-zinc-600 transition-colors" title="Actualizar">
    ...
    @if(!empty($recommendation))
        ...
        <div class="rounded-xl p-6 mb-4 
            ...
            @else bg-zinc-50 border-2 border-zinc-200
            @endif
        ">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
            <div class="bg-zinc-50 rounded-lg p-3 text-center">
                <div class="text-zinc-500 text-xs">NDVI</div>
                <div class="font-semibold {{ $recommendation['ndvi'] !== null && $recommendation['ndvi'] < 0.4 ? 'text-amber-600' : 'text-zinc-900' }}">
                    {{ $recommendation['ndvi'] !== null ? number_format($recommendation['ndvi'], 3) : 'N/A' }}
                </div>
            </div>
            <div class="bg-zinc-50 rounded-lg p-3 text-center">
                <div class="text-zinc-500 text-xs">NDWI</div>
                <div class="font-semibold {{ $recommendation['ndwi'] !== null && $recommendation['ndwi'] < -0.1 ? 'text-amber-600' : 'text-zinc-900' }}">
            ...
            <div class="bg-zinc-50 rounded-lg p-3 text-center">
                <div class="text-zinc-500 text-xs">ET₀</div>
                <div class="font-semibold {{ $recommendation['et0'] !== null && $recommendation['et0'] > 6 ? 'text-amber-600' : 'text-zinc-900' }}">
