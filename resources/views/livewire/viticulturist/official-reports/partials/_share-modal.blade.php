    ...
    <div class="fixed z-50 inset-0 overflow-y-auto">
        <div class="flex items-center justify-center min-h-screen px-4">
            ...
            <div class="relative bg-white rounded-2xl shadow-2xl max-w-lg w-full p-6">
                <h3 class="text-2xl font-bold text-zinc-900 mb-4 flex items-center gap-2"><flux:icon icon="envelope" class="size-7" /> {{ __('Compartir Informe') }}</h3>
                <p class="text-zinc-600 mb-6">
                    {{ __('Envía este informe por email. El destinatario recibirá el PDF adjunto y un enlace para verificar su autenticidad.') }}
                </p>

                <div class="mb-4">
                    <label class="block text-sm font-semibold text-zinc-700 mb-2">{{ __('Email del destinatario') }}</label>
                    <input
                        type="email"
                ...
                <div class="mb-6">
                    <label class="block text-sm font-semibold text-zinc-700 mb-2">{{ __('Mensaje personalizado (opcional)') }}</label>
                    <textarea
                        wire:model="shareMessage"
                        rows="3"
                        class="w-full px-4 py-2 border border-zinc-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                        placeholder="{{ __('Añade un mensaje personal...') }}"
                    ></textarea>
                    <p class="text-xs text-zinc-500 mt-1">{{ __('Máximo 500 caracteres') }}</p>
                ...
                <div class="bg-blue-50 border-l-4 border-blue-500 p-4 rounded-r-lg mb-6">
                    <p class="text-sm text-blue-800">
                        <strong>{{ __('Incluye') }}:</strong> {{ __('PDF del informe + enlace de verificación QR') }}
                    </p>
                </div>
                ...
                <div class="flex justify-end space-x-3">
                    ...
                    >
                        ...
                        <span wire:loading.remove wire:target="shareReport">{{ __('Enviar Email') }}</span>
                        <span wire:loading wire:target="shareReport">{{ __('Enviando...') }}</span>
                    </button>
                </div>
            </div>
        </div>
