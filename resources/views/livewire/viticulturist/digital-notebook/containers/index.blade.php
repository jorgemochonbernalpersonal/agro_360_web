<div class="space-y-6 animate-fade-in">
    ...
    <x-agro.page-header
        ...
        :description="__('Gestiona tus contenedores y asígnalos a cosechas')"
    />
    ...
    <x-agro.stats-section key="notebook-containers-stats">
        <div class="grid grid-cols-2 gap-4">
            <x-agro.stat-card :label="__('Capacidad Total')" :value="number_format($stats['total_capacity'], 0) . ' kg'" icon="scale" color="green" />
            <x-agro.stat-card :label="__('Capacidad Usada')" :value="number_format($stats['total_used'], 0) . ' kg'" icon="archive-box" color="blue" />
        </div>
    ...
    <div class="flex items-center gap-3">
        ...
        <x-agro.search-input wire:model.live.debounce.300ms="search" :placeholder="__('Buscar por nombre, número de serie, parcela...')" />
    ...
    @if($search || $selectedCampaign || $selectedHarvest || $filterAvailability)
        <div class="flex flex-wrap items-center gap-2">
            ...
            @if($filterAvailability)
                <x-agro.filter-chip :label="$filterAvailability === 'available' ? __('Disponibles') : __('Asignados')" wireRemove="$set('filterAvailability', '')" />
            @endif
    ...
    <div wire:loading.remove wire:target="switchTab, search, selectedCampaign, selectedHarvest, filterAvailability, clearFilters, nextPage, previousPage">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                @foreach($containers as $container)
                    @php
                        ...
                        if ($container->isEmpty())      { $fillColor = 'green'; $fillLabel = __('Vacío'); }
                        elseif ($container->isFull())   { $fillColor = 'blue';  $fillLabel = __('Lleno'); }
                        else                            { $fillColor = 'yellow'; $fillLabel = __('Parcial'); }
                    @endphp
                    <x-agro.card
                    ...
                    >
                        <x-slot:header>
                            <div class="flex items-center gap-3">
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <p class="text-xs text-zinc-500">{{ __('Cantidad:') }} {{ $container->quantity }}</p>
                                </div>
                        ...
                        <div class="flex-1 space-y-4">
                            {{-- Ocupación --}}
                            <x-agro.progress-bar :percentage="$pct" :label="__('Ocupación')" :showValues="false" />

                            {{-- Peso --}}
                            <div class="grid grid-cols-2 gap-2">
                                <div class="bg-amber-50 rounded-xl p-3">
                                    <p class="text-[10px] font-semibold text-amber-400 uppercase tracking-widest mb-0.5">{{ __('Usado') }}</p>
                                ...
                                <div class="bg-zinc-50 rounded-xl p-3">
                                    <p class="text-[10px] font-semibold text-zinc-400 uppercase tracking-widest mb-0.5">{{ __('Capacidad') }}</p>
                            ...
                            @if($currentHarvest)
                                <div class="space-y-2 text-sm">
                                    <div class="flex items-center justify-between">
                                        ...
                                        <span class="text-zinc-700 font-medium truncate ml-2">{{ $currentHarvest->activity->plot->name ?? __('Sin parcela') }}</span>
                                    </div>
                                    <div class="flex items-center justify-between">
                            ...
                            @else
                                <div class="flex items-center gap-2 text-sm text-green-600">
                                    ...
                                    <span class="font-medium">{{ __('Disponible') }}</span>
                                </div>
                            @endif
                        </div>
        ...
        @else
            <x-agro.empty-state
                icon="archive-box"
                :title="__('No hay contenedores registrados')"
                :description="($search || $selectedCampaign || $selectedHarvest || $filterAvailability) ? __('No se encontraron contenedores con los filtros seleccionados') : __('Los contenedores aparecerán aquí cuando se registren')"
            >
                @if(!$search && !$selectedCampaign && !$selectedHarvest && !$filterAvailability)
                    <x-slot:action>
                        <flux:button href="{{ roleRoute('viticulturist.containers.create') }}" variant="primary" icon="plus">
