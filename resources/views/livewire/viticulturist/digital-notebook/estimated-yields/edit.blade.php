...
@if($estimatedYield)
    <div class="mb-6 p-4 bg-blue-50 border-l-4 border-blue-500 rounded-r-lg">
        <div class="flex items-start gap-3">
            ...
            <div>
                <h4 class="text-sm font-semibold text-blue-900">{{ __('Información de la Estimación') }}</h4>
                <p class="text-xs text-blue-800 mt-1">
                    {{ __('Parcela:') }} <strong>{{ $estimatedYield->plotPlanting->plot->name ?? __('Sin parcela') }}</strong> |
                    {{ __('Variedad:') }} <strong>{{ $estimatedYield->plotPlanting->grapeVariety->name ?? __('Sin variedad') }}</strong> |
                    {{ __('Campaña:') }} <strong>{{ $estimatedYield->campaign->name ?? __('Sin campaña') }}</strong>
                </p>
                @if($estimatedYield->hasActualYield())
                    <p class="text-xs text-green-700 mt-1 font-semibold">
                        ✓ {{ __('Rendimiento real disponible:') }} {{ number_format($estimatedYield->actual_total_yield, 2) }} kg
                    </p>
                @endif
            </div>
        </div>
...
<x-agro.form-card
    :title="__('Editar Rendimiento Estimado')"
    :description="__('Modifica la estimación de rendimiento')"
...
>
    <form wire:submit="update" class="space-y-8" data-cy="edit-estimated-yield-form">
        ...
        <x-agro.form-actions :back-url="roleRoute('viticulturist.digital-notebook.estimated-yields.index')" :submit-label="__('Actualizar Estimación')" />
    </form>
</x-agro.form-card>
</div>
