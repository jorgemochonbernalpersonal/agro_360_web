<x-agro.form-card
    :title="__('Editar Tratamiento Fitosanitario')"
    :description="__('Modifica los datos del tratamiento fitosanitario registrado')"
...
>
    <form wire:submit="update" class="space-y-8" data-cy="treatment-form">
        ...
        <x-agro.form-section :title="__('Producto Fitosanitario')">
            ...
            @if($this->selectedProduct && $this->applicationsThisCampaign > 0)
                <div class="mt-3 flex items-start gap-2.5 p-3 bg-amber-50 border border-amber-200 rounded-xl text-sm text-amber-800">
                    ...
                    <span>{{ __('Este producto se ha aplicado') }} <strong>{{ $this->applicationsThisCampaign }} {{ $this->applicationsThisCampaign === 1 ? __('vez') : __('veces') }}</strong> {{ __('más en esta campaña. Consulta la etiqueta del producto para verificar el número máximo de aplicaciones permitidas.') }}</span>
                </div>
            @endif
                        ...
                        @if($field_applicator_id)
                            ...
                            @if($selectedApplicator)
                                ...
                                @if($selectedApplicator->isRopoExpired())
                                    <p class="mt-1 text-xs text-red-600 font-medium">⚠️ {{ __('El ROPO de este aplicador ha vencido. Actualiza su ficha antes de guardar el tratamiento.') }}</p>
                                @endif
                            @endif
                        @else
        ...
        <x-agro.form-actions
            ...
            :submit-label="__('Actualizar Tratamiento')"
        />
