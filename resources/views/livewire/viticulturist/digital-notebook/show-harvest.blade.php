<div class="space-y-6 animate-fade-in">
    <x-agro.page-header
        :title="__('Detalle de Cosecha')"
        :description="__('Información completa de la cosecha y sus contenedores')"
    >
        <x-slot:actions>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <x-agro.card>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4">
                    <div>
                        <p class="text-xs font-medium text-zinc-500 uppercase tracking-wide">{{ __('Parcela') }}</p>
                        <p class="text-sm font-semibold text-zinc-900 mt-0.5">{{ $harvest->activity->plot->name ?? __('Sin parcela') }}</p>
                    </div>

                    <div>
                        <p class="text-xs font-medium text-zinc-500 uppercase tracking-wide">{{ __('Variedad') }}</p>
                        @if($harvest->plotPlanting->name)
                            <p class="text-xs text-purple-600 font-medium mt-0.5">{{ $harvest->plotPlanting->name }}</p>
                        @endif
                        <p class="text-sm font-semibold text-zinc-900">{{ $harvest->plotPlanting->grapeVariety->name ?? __('Sin variedad') }}</p>
                    </div>

                    <div>
                        <p class="text-xs font-medium text-zinc-500 uppercase tracking-wide">{{ __('Campaña') }}</p>
                        <p class="text-sm font-semibold text-zinc-900 mt-0.5">{{ $harvest->activity->campaign->name ?? __('Sin campaña') }}</p>
                    </div>

                    <div>
                        <p class="text-xs font-medium text-zinc-500 uppercase tracking-wide">{{ __('Fecha de Cosecha') }}</p>
                    ...
                    <div>
                        <p class="text-xs font-medium text-zinc-500 uppercase tracking-wide">{{ __('Peso Total') }}</p>
                        <p class="text-lg font-bold text-purple-700 mt-0.5">{{ number_format($harvest->total_weight, 2) }} kg</p>
                    </div>

                    @if($harvest->yield_per_hectare)
                        <div>
                            <p class="text-xs font-medium text-zinc-500 uppercase tracking-wide">{{ __('Rendimiento') }}</p>
            ...
            @if($harvest->baume_degree || $harvest->brix_degree || $harvest->acidity_level || $harvest->ph_level || $harvest->color_rating || $harvest->aroma_rating || $harvest->health_status)
                <x-agro.card>
                    <x-slot:header>
                        <flux:heading size="sm">{{ __('Calidad de la Uva') }}</flux:heading>
                    </x-slot:header>

                    <div class="grid grid-cols-2 md:grid-cols-4 gap-x-6 gap-y-4">
                        @if($harvest->baume_degree)
                            <div>
                                <p class="text-xs font-medium text-zinc-500 uppercase tracking-wide">{{ __('Grados Baumé') }}</p>
                        ...
                        @if($harvest->brix_degree)
                            <div>
                                <p class="text-xs font-medium text-zinc-500 uppercase tracking-wide">{{ __('Grados Brix') }}</p>
                        ...
                        @if($harvest->acidity_level)
                            <div>
                                <p class="text-xs font-medium text-zinc-500 uppercase tracking-wide">{{ __('Acidez Total') }}</p>
                        ...
                        @if($harvest->ph_level)
                            <div>
                                <p class="text-xs font-medium text-zinc-500 uppercase tracking-wide">{{ __('pH') }}</p>
                        ...
                        @if($harvest->color_rating)
                            <div>
                                <p class="text-xs font-medium text-zinc-500 uppercase tracking-wide">{{ __('Color') }}</p>
                        ...
                        @if($harvest->aroma_rating)
                            <div>
                                <p class="text-xs font-medium text-zinc-500 uppercase tracking-wide">{{ __('Aroma') }}</p>
                        ...
                        @if($harvest->health_status)
                            <div>
                                <p class="text-xs font-medium text-zinc-500 uppercase tracking-wide">{{ __('Estado de Salud') }}</p>
            ...
            @if($harvest->destination_type || $harvest->destination || $harvest->buyer_name || $harvest->price_per_kg || $harvest->total_value)
                <x-agro.card>
                    <x-slot:header>
                        <flux:heading size="sm">{{ __('Destino y Valor Económico') }}</flux:heading>
                    </x-slot:header>

                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4">
                        @if($harvest->destination_type)
                            <div>
                                <p class="text-xs font-medium text-zinc-500 uppercase tracking-wide">{{ __('Tipo de Destino') }}</p>
                        ...
                        @if($harvest->destination)
                            <div>
                                <p class="text-xs font-medium text-zinc-500 uppercase tracking-wide">{{ __('Destino') }}</p>
                        ...
                        @if($harvest->buyer_name)
                            <div>
                                <p class="text-xs font-medium text-zinc-500 uppercase tracking-wide">{{ __('Comprador') }}</p>
                        ...
                        @if($harvest->price_per_kg)
                            <div>
                                <p class="text-xs font-medium text-zinc-500 uppercase tracking-wide">{{ __('Precio por kg') }}</p>
                        ...
                        @if($harvest->total_value)
                            <div class="sm:col-span-2">
                                <p class="text-xs font-medium text-zinc-500 uppercase tracking-wide">{{ __('Valor Total') }}</p>
            ...
            <x-agro.card>
                <x-slot:header>
                    <div class="flex items-center justify-between">
                        <flux:heading size="sm">{{ __('Contenedor') }}</flux:heading>
                        <flux:button
                ...
                @if($harvest->container)
                    @php
                        ...
                        $containerLabel = $container->isEmpty() ? __('Vacío') : ($container->isFull() ? __('Lleno') : __('Parcial'));
                    @endphp
                    ...
                    <div class="space-y-4">
                        ...
                        <x-agro.progress-bar
                            ...
                            :label="__('Ocupación del contenedor')"
                ...
                @else
                    <x-agro.empty-state
                        icon="archive-box"
                        :message="__('Sin contenedor asignado')"
                        :description="__('Esta cosecha no tiene contenedor asignado')"
                    >
                        <x-slot:action>
                            <flux:button
                                :href="roleRoute('viticulturist.containers.create') . '?harvest_id=' . $harvest->id"
            ...
            <x-agro.card>
                <x-slot:header>
                    <flux:heading size="sm">{{ __('Información de la Actividad') }}</flux:heading>
                </x-slot:header>

                <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4">
                    <div>
                        <p class="text-xs font-medium text-zinc-500 uppercase tracking-wide">{{ __('Fecha de Actividad') }}</p>
                        <p class="text-sm font-semibold text-zinc-900 mt-0.5">{{ $harvest->activity->activity_date->format('d/m/Y') }}</p>
                    </div>

                    @if($harvest->activity->crew)
                        <div>
                            <p class="text-xs font-medium text-zinc-500 uppercase tracking-wide">{{ __('Equipo') }}</p>
                    ...
                    @if($harvest->activity->crewMember?->viticulturist)
                        <div>
                            <p class="text-xs font-medium text-zinc-500 uppercase tracking-wide">{{ __('Viticultor') }}</p>
                    ...
                    @if($harvest->activity->machinery)
                        <div>
                            <p class="text-xs font-medium text-zinc-500 uppercase tracking-wide">{{ __('Maquinaria') }}</p>
                    ...
                    @if($harvest->activity->weather_conditions)
                        <div>
                            <p class="text-xs font-medium text-zinc-500 uppercase tracking-wide">{{ __('Condiciones Meteorológicas') }}</p>
                    ...
                    @if($harvest->activity->temperature)
                        <div>
                            <p class="text-xs font-medium text-zinc-500 uppercase tracking-wide">{{ __('Temperatura') }}</p>
                    ...
                    @if($harvest->activity->notes)
                        <div class="sm:col-span-2">
                            <p class="text-xs font-medium text-zinc-500 uppercase tracking-wide">{{ __('Notas') }}</p>
        ...
        <div class="space-y-4">
            ...
            @if($estimatedYield)
                <x-agro.card>
                    <x-slot:header>
                        <flux:heading size="sm">{{ __('Rendimiento Estimado') }}</flux:heading>
                    </x-slot:header>

                    <div class="space-y-3">
                        <div>
                            <p class="text-xs font-medium text-zinc-500 uppercase tracking-wide">{{ __('Estimado') }}</p>
                        ...
                        @if($estimatedYield->hasActualYield())
                            <div>
                                <p class="text-xs font-medium text-zinc-500 uppercase tracking-wide">{{ __('Real') }}</p>
                            ...
                            @if($estimatedYield->variance_percentage)
                                <div>
                                    <p class="text-xs font-medium text-zinc-500 uppercase tracking-wide">{{ __('Diferencia') }}</p>
            ...
            @if($harvestLimitInfo)
                <x-agro.card>
                    <x-slot:header>
                        <flux:heading size="sm">{{ __('Estado vs Límite') }}</flux:heading>
                    </x-slot:header>

                    <div class="space-y-4">
                        <x-agro.progress-bar
                            :percentage="$harvestLimitInfo['percentage']"
                            :label="__('Uso del límite de plantación')"
                        ...
                        <div class="space-y-2 text-sm">
                            <div class="flex justify-between">
                                <span class="text-zinc-500">{{ __('Límite máximo') }}</span>
                                <span class="font-semibold text-zinc-900">{{ number_format($harvestLimitInfo['limit'], 2) }} kg</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-zinc-500">{{ __('Total cosechado') }}</span>
                        ...
                        @if($harvestLimitInfo['exceeds'])
                            <flux:callout variant="danger" icon="exclamation-triangle">
                                <flux:callout.text>{{ __('El total cosechado excede el límite de la plantación.') }}</flux:callout.text>
                            </flux:callout>
                        @elseif($harvestLimitInfo['percentage'] > 80)
                            <flux:callout variant="warning" icon="exclamation-circle">
                                <flux:callout.text>{{ __('Se ha utilizado más del 80% del límite.') }}</flux:callout.text>
                            </flux:callout>
                        @endif
                    </div>
                </x-agro.card>
                ...
                <x-agro.card class="border-amber-200 bg-amber-50">
                    <x-slot:header>
                        <div class="flex items-center gap-2">
                            ...
                            <flux:heading size="sm">{{ __('Historial de Ediciones') }}</flux:heading>
                        </div>
                    </x-slot:header>

                    <div class="space-y-2 text-sm">
                        <div>
                            <p class="text-xs font-medium text-amber-700">{{ __('Editada el') }}</p>
                            <p class="font-semibold text-amber-900">{{ $harvest->edited_at->format('d/m/Y H:i') }}</p>
                        </div>
                        @if($harvest->editor)
                            <div>
                                <p class="text-xs font-medium text-amber-700">{{ __('Por') }}</p>
                                <p class="font-semibold text-amber-900">{{ $harvest->editor->name }}</p>
                            </div>
                        @endif
                        @if($harvest->edit_notes)
                            <div>
                                <p class="text-xs font-medium text-amber-700">{{ __('Motivo') }}</p>
            ...
            <x-agro.card>
                <x-slot:header>
                    <flux:heading size="sm">{{ __('Registro') }}</flux:heading>
                </x-slot:header>

                <div class="space-y-2 text-sm">
                    <div class="flex justify-between items-center">
                        <span class="text-zinc-500">{{ __('Creada') }}</span>
                        <span class="font-medium text-zinc-700">{{ $harvest->created_at->format('d/m/Y H:i') }}</span>
                    </div>
                    <div class="flex justify-between items-center">
                        <span class="text-zinc-500">{{ __('Actualizada') }}</span>
