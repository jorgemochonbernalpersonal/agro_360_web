...
class Index extends AbstractIndex
{
    ...
    public function updatingDisqualifiedFilter(): void
    {
        $this->resetPage();
    }
    ...
    protected function applyFilters(Builder $query): void
    {
        if ($this->campaignFilter) {
            $query->whereHas('batch', fn (Builder $q) => $q->where('campaign_id', $this->campaignFilter)
            );
        }

        if ($this->viticulturistFilter) {
            $query->whereHas('batch', fn (Builder $q) => $q->where('viticulturist_id', $this->viticulturistFilter)
            );
        }
        ...
        if ($this->search) {
            ...
            $query->where(function (Builder $q) use ($term) {
                $q->whereHas('batch.viticulturist', fn ($q2) => $q2->whereRaw('LOWER(name) LIKE ?', [$term])
                )
    ...
    {
        return ['harvest_start_date', 'desc'];
    }
    ...
    protected function viewData(mixed $entries): array
    {
        ...
        if ($this->campaignFilter) {
            ...
            $harvestsInCampaign = Harvest::with([
                ...
                'plotPlanting.plot',
                'batch.viticulturist',
            ])->whereHas('batch', fn ($q) => $q->where('winery_id', $wineryId)
