...
class Index extends Component
{
    ...
    public function render()
    {
        ...
        $rows = $allRows->filter(fn ($row) => $this->currentTab === 'delivered'
                ? $row['has_delivery']
                : ! $row['has_delivery']
        );
        ...
        if ($this->search) {
            ...
            $rows = $rows->filter(function ($row) use ($s) {
                if (str_contains(strtolower($row['variety']), $s)) {
                    return true;
                }
        ...
        if ($this->statusFilter) {
            $rows = match ($this->statusFilter) {
                'has_dispute' => $rows->filter(fn ($row) => $row['manual_deliveries']->contains(fn ($d) => $d->status === 'disputed' && ! $d->disqualified
                )
                ),
                'has_resolved' => $rows->filter(fn ($row) => $row['manual_deliveries']->contains(fn ($d) => $d->status === 'resolved' && ! $d->disqualified
                )
                ),
