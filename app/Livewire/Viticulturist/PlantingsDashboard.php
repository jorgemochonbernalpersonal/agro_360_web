...
class PlantingsDashboard extends Component
{
    public function render()
    {
        ...
        $expiringCertifications = $plantings->flatMap(fn ($p) => $p->certifications->filter(fn ($c) => $c->status === 'active' &&
                $c->expiry_date &&
                $c->expiry_date >= $now &&
        ...
        foreach ($plantings as $planting) {
            ...
            $expiringCerts = $planting->certifications->filter(fn ($c) => $c->expiry_date &&
                $c->expiry_date >= now() &&
