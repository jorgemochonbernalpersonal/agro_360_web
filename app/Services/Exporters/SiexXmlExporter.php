    ...
    public function exportFullNotebook(
        ...
        foreach ($activities as $activity) {
            ...
            if ($activity->activity_type === 'fertilization' && $activity->fertilization) {
                ...
                if ($f->nitrogen_uf !== null) {
                    $this->addElement($xml, $fert, 'NitrogenoUF', (string) $f->nitrogen_uf);
                }
                if ($f->phosphorus_uf !== null) {
                    $this->addElement($xml, $fert, 'FosforoUF', (string) $f->phosphorus_uf);
                }
                if ($f->potassium_uf !== null) {
                    $this->addElement($xml, $fert, 'PotasioUF', (string) $f->potassium_uf);
                }

                if ($f->manure_type) {
                    $this->addElement($xml, $fert, 'TipoEstiercol', $f->manure_type);
                }
                if ($f->burial_date) {
                    $this->addElement($xml, $fert, 'FechaEnterrado', $f->burial_date->format('Y-m-d'));
                }
                if ($f->emission_reduction_method) {
                    $this->addElement($xml, $fert, 'MetodoReduccion', $f->emission_reduction_method);
                }
            ...
            if ($activity->activity_type === 'harvest' && $activity->harvest) {
                ...
                $h = $activity->harvest;
                if ($h->transport_document_number) {
                    $this->addElement($xml, $harvestEl, 'DocumentoTransporte', $h->transport_document_number);
                }
                if ($h->destination_rega_code) {
                    $this->addElement($xml, $harvestEl, 'REGADestino', $h->destination_rega_code);
                }
                if ($h->vehicle_plate) {
                    $this->addElement($xml, $harvestEl, 'Matricula', $h->vehicle_plate);
                }
