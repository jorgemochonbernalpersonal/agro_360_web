    ...
    public function export(int $wineryId, int $vintage): string
    {
        ...
        foreach ($harvests as $h) {
            $rows[] = [
                ...
                'CANTIDAD'            => $this->formatQty(($h->total_weight ?? 0) / 100), // kg→hl approx.
        ...
        foreach ($external as $eg) {
            ...
            $rows[] = [
                ...
                'CANTIDAD'              => $this->formatQty($eg->total_weight_kg / 100),
                'UNIDAD_MEDIDA'         => self::UNIT_KG,
                'ORIGEN_DESTINO_NIF'    => '',
        ...
        foreach ($wines as $w) {
            ...
            $rows[] = [
                'TIPO_MOVIMIENTO'       => 'A15',
                'FECHA_OPERACION'       => $this->formatDate(now()->year($vintage)->endOfYear()->format('Y-m-d')),
                'CODIGO_NC'             => $ncCode,
                'DESCRIPCION_PRODUCTO'  => $description,
