...
class WineResource extends JsonResource
{
    public function toArray(Request $request): array
    {
        return [
            ...
            'notes' => $this->notes,
            'latest_fermentation' => $this->whenLoaded('fermentationControls', fn () => $this->fermentationControls->sortByDesc('control_date')->first()
                    ? new FermentationControlResource(
                        $this->fermentationControls->sortByDesc('control_date')->first()
                    )
                    : null
            ),
            'active_containers' => $this->whenLoaded('currentStates', fn () => $this->currentStates->map(fn ($s) => [
