...
class NotebookAccessTest extends SupervisorTestCase
{
    ...
    public function test_stats_count_requests_by_status(): void
    {
        ...
        Livewire::actingAs($supervisor)
            ->test(Index::class)
            ->assertViewHas('stats', fn ($s) => $s['total'] === 3 && $s['pending'] === 1 && $s['approved'] === 1 && $s['rejected'] === 1
            );
    }
    ...
    public function test_ghost_viticulturists_excluded_from_available_list(): void
    {
        ...
        Livewire::actingAs($supervisor)
            ->test(Index::class)
            ->call('openRequestModal')
            ->assertViewHas('availableForRequest', fn ($list) => $list->contains('id', $active->id) && ! $list->contains('id', $ghost->id)
            );
    }
    ...
    public function test_filter_by_status(): void
    {
        ...
        Livewire::actingAs($supervisor)
            ...
            ->assertViewHas('requests', fn ($p) => $p->contains('id', $r1->id) && ! $p->contains('id', $r2->id)
            );
    }
    // ── helpers ───────────────────────────────────────────────────────────────
