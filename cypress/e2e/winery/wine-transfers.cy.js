/**
 * Winery Wine Transfers (Trasvases de vino) - E2E Tests
 * Cubre: Index, Create, Edit
 *
 * Invariante clave: un trasvase sólo debe modificar wine_volume_liters,
 * nunca used_capacity (campo exclusivo de cosechas/uva).
 */

describe('Winery Wine Transfers', () => {
  beforeEach(() => {
    cy.loginAsWinery()
    cy.visit('/winery/wine-transfers')
    cy.waitForLivewire()
  })

  // ── Index ──────────────────────────────────────────────────────────────────

  describe('Index', () => {
    it('muestra la página de trasvases', () => {
      cy.url().should('include', '/winery/wine-transfers')
      cy.get('body').should('be.visible')
    })

    it('tiene botón para crear trasvase', () => {
      cy.get('a[href*="/winery/wine-transfers/create"]').should('be.visible')
    })

    it('filtra por búsqueda si el componente tiene campo', () => {
      cy.get('body').then(($body) => {
        const search = $body.find(
          '[wire\\:model\\.live\\.debounce\\.300ms="search"], [wire\\:model\\.live="search"], [wire\\:model="search"]'
        )
        if (search.length > 0) {
          cy.wrap(search.first()).type('test', { force: true })
          cy.waitForLivewire()
          cy.wait(600)
        }
        cy.get('body').should('be.visible')
      })
    })
  })

  // ── Create ─────────────────────────────────────────────────────────────────

  describe('Create', () => {
    beforeEach(() => {
      cy.visit('/winery/wine-transfers/create')
      cy.waitForLivewire()
    })

    it('muestra el formulario de trasvase', () => {
      cy.url().should('include', '/winery/wine-transfers/create')
      cy.getByWireModel('quantity').should('exist')
      cy.getByWireModel('transfer_date').should('exist')
      cy.getByWireModel('transfer_type').should('exist')
    })

    it('valida campos requeridos al guardar sin datos', () => {
      cy.get('[data-cy="submit-button"], button[type="submit"]').first().click({ force: true })
      cy.wait(2000)
      cy.url().should('include', '/winery/wine-transfers/create')
    })

    it('crea trasvase si hay vinos y contenedores', () => {
      cy.getByWireModel('wine_id').then(($sel) => {
        const opts = $sel.find('option').filter((i, el) => el.value !== '')
        if (opts.length === 0) {
          cy.log('Sin vinos disponibles, omitiendo test de creación')
          return
        }

        cy.wrap($sel).select(opts.first().val(), { force: true })
        cy.waitForLivewire()

        cy.getByWireModel('to_container_id').then(($destSel) => {
          const destOpts = $destSel.find('option').filter((i, el) => el.value !== '')
          if (destOpts.length === 0) {
            cy.log('Sin contenedores destino disponibles, omitiendo')
            return
          }

          cy.wrap($destSel).select(destOpts.first().val(), { force: true })
          cy.waitForLivewire()

          cy.getByWireModel('quantity').clear().type('10')
          cy.getByWireModel('transfer_date').clear().type(new Date().toISOString().split('T')[0])

          // Seleccionar unidad de medida si la tiene
          cy.getByWireModel('unit_of_measurement_id').then(($uomSel) => {
            const uomOpts = $uomSel.find('option').filter((i, el) => el.value !== '')
            if (uomOpts.length > 0) {
              cy.wrap($uomSel).select(uomOpts.first().val(), { force: true })
            }
          })

          cy.get('[data-cy="submit-button"], button[type="submit"]').first().click({ force: true })
          cy.wait(3000)
          cy.waitForLivewire()
          cy.url().should('include', '/winery/wine-transfers')
        })
      })
    })

    it('muestra error si cantidad supera la capacidad disponible del destino', () => {
      cy.getByWireModel('wine_id').then(($sel) => {
        const opts = $sel.find('option').filter((i, el) => el.value !== '')
        if (opts.length === 0) {
          cy.log('Sin vinos, omitiendo')
          return
        }

        cy.wrap($sel).select(opts.first().val(), { force: true })
        cy.waitForLivewire()

        cy.getByWireModel('to_container_id').then(($destSel) => {
          const destOpts = $destSel.find('option').filter((i, el) => el.value !== '')
          if (destOpts.length === 0) {
            cy.log('Sin contenedores, omitiendo')
            return
          }

          cy.wrap($destSel).select(destOpts.first().val(), { force: true })
          cy.waitForLivewire()

          // Cantidad absurdamente alta
          cy.getByWireModel('quantity').clear().type('999999999')
          cy.getByWireModel('transfer_date').clear().type(new Date().toISOString().split('T')[0])

          cy.getByWireModel('unit_of_measurement_id').then(($uomSel) => {
            const uomOpts = $uomSel.find('option').filter((i, el) => el.value !== '')
            if (uomOpts.length > 0) {
              cy.wrap($uomSel).select(uomOpts.first().val(), { force: true })
            }
          })

          cy.get('[data-cy="submit-button"], button[type="submit"]').first().click({ force: true })
          cy.wait(2000)
          cy.url().should('include', '/winery/wine-transfers/create')
        })
      })
    })
  })

  // ── Edit ───────────────────────────────────────────────────────────────────

  describe('Edit', () => {
    it('navega a edición si existe un trasvase', () => {
      cy.get('body').then(($body) => {
        const editLinks = $body.find('a[href*="/winery/wine-transfers/"][href*="/edit"]')
        if (editLinks.length === 0) {
          cy.log('Sin trasvases existentes, omitiendo test de edición')
          return
        }

        cy.wrap(editLinks.first()).click({ force: true })
        cy.waitForLivewire()
        cy.url().should('include', '/edit')
        cy.getByWireModel('quantity').should('exist')
      })
    })

    it('guarda cambios de edición', () => {
      cy.get('body').then(($body) => {
        const editLinks = $body.find('a[href*="/winery/wine-transfers/"][href*="/edit"]')
        if (editLinks.length === 0) {
          cy.log('Sin trasvases existentes, omitiendo')
          return
        }

        cy.wrap(editLinks.first()).click({ force: true })
        cy.waitForLivewire()

        cy.getByWireModel('notes').clear().type(`Editado Cypress ${Date.now()}`)

        cy.get('[data-cy="submit-button"], button[type="submit"]').first().click({ force: true })
        cy.wait(2000)
        cy.waitForLivewire()
        cy.url().should('include', '/winery/wine-transfers')
      })
    })
  })
})
