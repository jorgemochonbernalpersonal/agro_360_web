...
class WineryProductLotsSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        foreach ($wines as $wine) {
            ...
            if (stripos($wine->name, 'crianza') !== false)       $aging = 'crianza';
            elseif (stripos($wine->name, 'reserva') !== false)   $aging = 'reserva';
            elseif (stripos($wine->name, 'barrica') !== false)   $aging = 'barrica';

            $basePrice = $priceMap[$wineTypeLot][$aging] ?? 9.00;
            // Añadas más antiguas tienen premium
            if ($wine->vintage && $wine->vintage <= 2022) $basePrice *= 1.3;
            if ($wine->vintage && $wine->vintage === 2023) $basePrice *= 1.15;
            if ($wine->is_organic) $basePrice *= 1.10;
